 ...
 *
 * <p>
 *  数字转换和解析的其他实用方法<p>主要用于框架内部使用;考虑Apache的Commons Lang为更全面的数字实用程序套件
 * 
 * 
...
public abstract class NumberUtils {
	 ...
	 * <p>
	 * 标准数字类型(全部不变)：字节,短整型,长整型,BigInteger,Float,Double,BigDecimal
	 * 
	 */
	 ...
	 * <p>
	 *  在给定数字返回长值之前,请检查{@code BigInteger} / {@ code BigDecimal}长溢出
	 * 
	 * 
	 * @param number the number to convert
	 * @param targetClass the target class to convert to
	 ...
	 * <p>
	 *  将给定的{@code text}解析为给定目标类的{@link Number}实例,使用相应的{@code decode} / {@code valueOf}方法<p>在尝试之前修剪输入{@code String}
	 * 解析数字<p>支持十六进制格式的数字(带前导"0x","0X"或"#")。
	 * 
	 * 
	 ...
	 * <p>Trims the input {@code String} before attempting to parse the number.
	 * <p>
	 * 在给定的目标类的{@link Number}实例中解析给定的{@code text},使用提供的{@link NumberFormat} <p>修剪输入{@code String},然后再尝试解析该数字
	 * 。
	 * 
	 * 
	 * @param text the text to convert
	 ...
	 * <p>
	 *  确定给定的{@code值} String是否表示十六进制数,即需要传递到{@code Integerdecode}而不是{@code IntegervalueOf}等
	 * 
	 */
	 ...
	 * <p>
	 *  从提供的{@link String}值解码{@link javamathBigInteger} <p>支持十进制,十六进制和八进制符号
	 * 
