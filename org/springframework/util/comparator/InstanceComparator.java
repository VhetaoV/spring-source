 ...
 *
 * <p>
 * 比较基于任意类顺序的对象允许对象根据他们继承的类的类型进行排序,例如：该比较器可用于对列表{@code Number}进行排序,以使{@code Long}发生之前{@code整数}
 * 
 *  <p>在比较期间只考虑指定的{@code instanceOrder}类如果两个对象都是有序类型的实例,则该比较器将返回{@code 0}如果需要额外的排序,请考虑与{@link CompoundComparator}
 * 组合。
 * 
 * @author Phillip Webb
 * @since 3.2
