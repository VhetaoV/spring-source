 ...
 *
 * <p>
 *  提供在{@link WebApplicationContext}中运行的应用程序对象的方便的超类提供{@code getWebApplicationContext()},{@code getServletContext()}
 * 和{@code getTempDir()}访问器。
 * 
 * 注意：通常建议使用单个回调接口来实现所需的实际回调。这个广泛的基类主要是为了在框架内使用,在{@link ServletContext}访问等通常需要的情况下
 * 
 * 
...
public abstract class WebApplicationObjectSupport extends ApplicationObjectSupport implements ServletContextAware {
	 ...
	 * <p>
	 *  覆盖基类行为以强制在ApplicationContext中运行所有访问器将抛出IllegalStateException,如果不在上下文中运行
	 * 
	 * 
	 ...
	 * <p>
	 *  如果给定的ApplicationContext是{@link WebApplicationContext},调用{@link #initServletContext(javaxservletServletContext)}
	 * 。
	 * 
	 */
	@Override
	 ...
	 * <p>
	 * 子类可以根据此应用程序对象在<p>中运行的ServletContext进行自定义初始化。
	 * 默认实现为空由{@link #initApplicationContext(orgspringframeworkcontextApplicationContext)}调用)以及{@link #setServletContext(javaxservletServletContext)}
	 * )。
	 * 子类可以根据此应用程序对象在<p>中运行的ServletContext进行自定义初始化。
	 * 
	 * 
	 ...
	 * <p>
	 *  将当前应用程序上下文返回为WebApplicationContext <p> <b>注意：</b>仅在实际需要访问WebApplicationContext特定功能时才使用此选项。
	 * 最好使用{@code getApplicationContext()}或{@code getServletContext()} else ,以便能够在非WebApplicationContext环境中运
	 * 行。
	 *  将当前应用程序上下文返回为WebApplicationContext <p> <b>注意：</b>仅在实际需要访问WebApplicationContext特定功能时才使用此选项。
	 * 
	 * 
