...
public abstract class WebUtils {
	 ...
	 * <p>
	 * 标准Servlet 23+包含URI和路径的规范请求属性<p>如果通过RequestDispatcher包含,当前资源将看到始发请求,它自己的URI和路径将被显示为请求属性
	 * 
	 */
	 ...
	 * <p>
	 *  前向URI和路径的标准Servlet 24+规范请求属性<p>如果通过RequestDispatcher转发,则当前资源将看到其自己的URI和路径起始URI和路径将作为请求属性公开
	 * 
	 */
	 ...
	 * <p>
	 *  标准Servlet 23+错误页面的规范请求属性<p>要暴露给标记为错误页面的JSP,直接转发到它们,而不是通过servlet容器的错误页解析机制
	 * 
	 */
	 ...
	 * <p>
	 *  内容类型中charset子句的前缀String："; charset ="
	 * 
	 */
	 ...
	 * <p>
	 * 根据Servlet规范,{@code requestgetCharacterEncoding}返回{@code null}时使用的默认字符编码
	 * 
	 * 
	 ...
	 * <p>
	 *  标准Servlet规范上下文属性,指定当前Web应用程序的临时目录,类型为{@code javaioFile}
	 * 
	 */
	 ...
	 * <p>
	 *  servlet上下文级别的HTML转义参数(即{@code webxml}中的上下文参数))："defaultHtmlEscape"
	 * 
	 */
	 ...
	 * <p>
	 *  在servlet上下文级别(即{@code webxml}中的上下文参数)中使用对HTML转义参数的响应编码)："responseEncodedHtmlEscape"
	 * 
	 * 
	 * @since 4.1.2
	 */
	 ...
	 * <p>
	 *  servlet上下文级别的Web应用程序根密钥参数(即{@code webxml}中的上下文参数))："webAppRootKey"
	 * 
	 */
	 ...
	 * <p>
	 * 将系统属性设置为Web应用程序根目录可以使用{@code webxml}中的"webAppRootKey"上下文参数定义系统属性的关键字默认为"webapproot"<p>可用于支持替换的工具{@code SystemgetProperty}
	 * 值,如日志文件位置中的log4j的"$ {key}"语法。
	 * 
	 * 
	 ...
	 * <p>
	 *  删除指向Web应用程序根目录的系统属性要关闭Web应用程序时调用
	 * 
	 * 
	 * @param servletContext the servlet context of the web application
	 ...
	 * <p>
	 *  返回是否为Web应用程序启用了默认HTML转义,即{@code webxml}(如果有)中的"defaultHtmlEscape"上下文参数的值将返回{@code false},如果没有明确的默认值。
	 * 
	 * 
	 * @param servletContext the servlet context of the web application
	 ...
	 * <p>
	 * 返回是否为Web应用程序启用了默认HTML转义,即{@code webxml}(如果有)中的"defaultHtmlEscape"上下文参数的值<p>此方法区分了根本没有指定的参数和实际布尔值指定,允许
	 * 具有上下文特定的默认值,以防在全局级别没有设置。
	 * 
	 * 
	 * @param servletContext the servlet context of the web application
	 ...
	 * default in case of no setting at the global level.
	 * <p>
	 * 返回是否应在HTML转义字符时使用响应编码,因此只能使用UTF- *编码转义XML标记重要字符此选项对于具有ServletContext参数的Web应用程序启用,即{responseEncodedHtmlEscape"上下文参数的值在{ @code webxml}
	 * (如果有的话)<p>这个方法区分了没有指定的参数和指定的实际布尔值,允许在全局级没有设置的情况下具有上下文特定的默认值。
	 * 
	 * 
	 * @param servletContext the servlet context of the web application
	 ...
	 * which returns null).
	 * <p>
	 * 返回Web应用程序中给定路径的真实路径,如servlet容器所提供的<p>如果路径尚未以斜杠开头,则会抛出斜杠,如果路径无法解析为资源,则会抛出FileNotFoundException(与Servle
	 * tContext的{@code getRealPath}相反,返回null)。
	 * 
	 * 
	 * @param servletContext the servlet context of the web application
	 ...
	 * <p>
	 *  检查给定请求的给定名称的会话属性如果没有会话或如果会话没有此类属性,则返回null如果以前没有,则不创建新会话！
	 * 
	 * 
	 * @param request current HTTP request
	 ...
	 * <p>
	 *  检查给定请求的给定名称的会话属性如果没有会话或如果会话没有此类属性,则抛出异常如果以前没有,则不创建新会话！
	 * 
	 * 
	 * @param request current HTTP request
	 * @param name the name of the session attribute
	 ...
	 * <p>
	 * 将给定名称的会话属性设置为给定值如果值为空,则删除会话属性(如果存在会话)如果不需要则不创建新会话！
	 * 
	 * 
	 * @param request current HTTP request
	 * @param name the name of the session attribute
	 ...
	 * <p>
	 *  获取指定的会话属性,创建和设置新属性(如果没有找到)给定的类需要有一个public no-arg构造函数有用于Web层中的按需状态对象,如购物车
	 * 
	 * 
	 ...
	 * 发现mutex属性,则返回到HttpSession本身<p>会话互斥体在会话的整个生命周期内保证是相同的对象,可用在由{@code SESSION_MUTEX_ATTRIBUTE}常量定义的关键字下,它
	 * 用作对当前会话进行锁定的同步的安全引用<p>在许多情况下,HttpSession引用本身也是一个安全的互斥体,因为它始终是相同的活动逻辑会话的相同对象引用但是,不能保证跨不同的servlet容器;唯一的
	 * 100％安全方式是会话互斥体。
	 * 
	 * 
	 ...
	 * <p>
	 *  确定给定的请求是否是包含请求,即不是从外部进入的顶级HTTP请求<p>检查是否存在"javaxservletincluderequest_uri"请求属性可以检查只存在于包含的任何请求属性请求
	 * 
	 * 
	 ...
	 * <p>
	 * 在Servlet 23规范中定义的键下,将Servlet规范的错误属性显示为{@link javaxservlethttpHttpServletRequest}属性,用于直接呈现的错误页面,而不是通过S
	 * ervlet容器的错误页面解析：{@code javaxservleterrorstatus_code},{@code javaxservleterrorexception_type },{@code javaxservleterrormessage}
	 * ,{@code javaxservleterrorexception},{@code javaxservleterrorrequest_uri},{@code javaxservleterrorservlet_name}
	 *  <p>如果已经存在,不覆盖值,以便在<p>暴露之前明确暴露的属性值状态码200默认情况下显式设置"javaxservleterrorstatus_code"属性(之前或之后),以显示不同的状态码。
	 * 
	 * 
	 * @param request current servlet request
	 ...
	 * {@code javax.servlet.error.servlet_name}.
	 * <p>
	 *  在Servlet 23规范中定义的键下,将Servlet规范的错误属性清除为{@code javaxservlethttpHttpServletRequest}属性：{@code javaxservleterrorstatus_code}
	 * ,{@code javaxservleterrorexception_type},{@code javaxservleterrormessage},{@code javaxservleterrorexception}
	 * ,{@code javaxservleterrorrequest_uri},{@code javaxservleterrorservlet_name}。
	 * 
	 * 
	 * @param request current servlet request
	 */
	 ...
	 * <p>
	 *  将给定的Map作为请求属性公开,使用键作为属性名称,并将值作为对应的属性值。Keys需要是Strings
	 * 
	 * 
	 * @param request current HTTP request
	 ...
	 * <p>
	 *  检索具有给定名称的第一个cookie请注意,多个Cookie可以具有相同的名称,但不同的路径或域
	 * 
	 * 
	 * @param request current servlet request
	 ...
	 * name + ".y").
	 * <p>
	 * 检查特定的输入类型="提交"参数是否在请求中发送,通过按钮(直接使用名称)或通过图像(名称+"x"或名称+"y")发送
	 * 
	 * 
	 * @param request current HTTP request
	 ...
	 * <p>
	 *  从给定的请求参数获取命名参数<p>有关查找算法的描述,请参阅{@link #findParameterValue(javautilMap,String)}
	 * 
	 * 
	 * @param request current HTTP request
	 ...
	 * </ol>
	 * <p>
	 *  从给定的请求参数获取命名参数<p>此方法将尝试使用以下算法获取参数值：
	 * <ol>
	 * <li>尝试仅使用给定的<i>逻辑</i>名称获取参数值。
	 * 此处理表单<tt> logicalName = value </tt>的参数对于常规参数,例如使用隐藏的HTML表单域提交,这将返回请求的值</li> <li>尝试从参数名称获取参数值,其中请求中的参数
	 * 名称的格式为<tt> logicalName_value = xyz </tt>带有"_"作为配置的分隔符此处理使用HTML表单提交按钮提交的参数值</li> <li>如果上一步中获得的值具有"x"或"
	 * y"后缀,则删除该句柄处理值使用HTML表单图像按钮提交在这种情况下,请求中的参数实际上将是<tt> logicalName_valuex = 123 </tt></李>。
	 * <li>尝试仅使用给定的<i>逻辑</i>名称获取参数值。
	 * </ol>
	 * 
	 ...
	 * <p>
	 * 返回包含给定前缀的所有参数的地图将单个值映射到String,将多个值映射到String数组<p>例如,使用前缀"spring_","spring_param1"和"spring_param2"导致"pa
	 * ram1"和"param2"作为键。
	 * 
	 * 
	 ...
	 * <p>
	 *  从给定的请求URL路径中提取URL文件名正确地解析诸如"/ products / viewhtml"这样的嵌套路径
	 * 
	 * 
	 ...
	 * <p>
	 *  从给定的请求URL路径中提取完整的URL文件名(包括文件扩展名)正确地解析诸如"/ products / viewhtml"之类的嵌套路径,并删除任何路径和或查询参数
	 * 
	 * 
	 ...
	 * <p>
	 * 用矩阵变量解析给定的字符串示例字符串看起来像这样{@code"q1 = a; q1 = b; q2 = a,b,c"}生成的映射将包含密钥{@code"q1"}和{值为{@code ["a","b"]}
	 * 和{@code ["a","b","c"]}的@code"q2"}。
	 * 
	 * 
	 ...
	 * <p>
	 *  根据允许的来源列表检查给定的请求原始文件包含"*"的列表表示允许所有来源空列表表示只允许相同的来源
	 * 
	 * 
	 ...
	 * <p>
	 *  基于{@code Origin},{@code Host},{@code Forwarded}和{@code X-Forwarded-Host}标题,检查请求是否是同源的请求
	 * 
