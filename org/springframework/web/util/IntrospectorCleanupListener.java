 ...
 *
 * <p>
 * 在Web应用程序关闭时刷新JDK的{@link javabeansIntrospector JavaBeans Introspector}缓存的侦听器在{@code webxml}中注册此侦听器,以确保
 * Web应用程序类加载器及其加载类的正确发布。
 * 
 *  <p> <b>如果JavaBeans Introspector用于分析应用程序类,则系统级Introspector缓存将持有对这些类的硬引用因此,这些类和Web应用程序类加载器不会在Web上垃圾回收应
 * 用程序关闭！</b>此侦听器执行适当的清理,以允许垃圾回收生效。
 * 
 * <p>不幸的是,清理Introspector的唯一方法是刷新整个缓存,因为没有办法专门确定应用程序在此处引用的类。这将删除服务器中所有其他应用程序的缓存内省结果
 * 
 *  <p>请注意,在应用程序中使用Spring的Bean基础架构时,此侦听器不是必需的,因为Spring自己的内省结果缓存将立即从JavaBeans Introspector缓存中刷新分析的类,并且只能在
 * 应用程序自己的ClassLoader。
 * 
 * <b>尽管Spring本身并没有创建JDK Introspector漏洞,但请注意,这个监听器应该在Spring框架类本身驻留在"常用"ClassLoader(例如系统ClassLoader))的场景中
 * 
