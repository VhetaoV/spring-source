 ...
 *
 * <p>
 *  标签库相关代码的实用类,揭示诸如将{@link String Strings}翻译成Web作用域的功能
 * 
 * <p>
 * <ul>
 * <li> {@ code page}将被转换为{@link javaxservletjspPageContext#PAGE_SCOPE PageContextPAGE_SCOPE} <li> {@ code request}
 * 将被转换为{@link javaxservletjspPageContext#REQUEST_SCOPE PageContextREQUEST_SCOPE} <li> {@ code session}将
 * 被转换为{@link javaxservletjspPageContext#SESSION_SCOPE PageContextSESSION_SCOPE} <li> {@ code application}
 * 将被转换为{@link javaxservletjspPageContext#APPLICATION_SCOPE PageContextAPPLICATION_SCOPE}。
 * </ul>
 * 
 * 
...
public abstract class TagUtils {
	 ...
	 * <p>
	 *  确定给定输入的范围{@code String} <p>如果{@code String}与"request","session","page"或"application"不匹配,则该方法将返回{@link PageContext# PAGE_SCOPE}
	 * 。
	 * 
	 * 
	 ...
	 * if not.
	 * <p>
	 *  确定提供的{@link Tag}是否具有提供类型的任何祖代标记
	 * 
	 * 
	 * @param tag the tag whose ancestors are to be checked
	 * @param ancestorTagClass the ancestor {@link Class} being searched for
