 ...
 *
 * <p>
 *  ViewRendererServlet是一个桥接Servlet,主要用于Portlet MVC的支持
 * 
 * <p>为了使用Portlet,此Servlet是必需的,强制Portlet容器将PortletRequest转换为ServletRequest,ServletRequest在通过PortletReque
 * stDispatcher包含资源时必须执行。
 * 这样可以重复使用整个基于Servlet的View支持甚至在Portlet环境中。
 * 
 *  <p>通过"viewRendererUrl"属性,可以在DispatcherPortlet中配置桥接Servlet的实际映射默认值为"/ WEB-INF / servlet / view",仅适用于内
 * 部资源调度。
 * 
 * 
...
public class ViewRendererServlet extends HttpServlet {
	 ...
	 * <p>
	 *  请求属性来保存当前的Web应用程序上下文否则只有全局Web应用程序上下文可以通过标签等获得
	 * 
	 * 
	 * @see #renderView
	 */
	 ...
	 * <p>
	 *  处理此请求,处理异常实际的事件处理由抽象{@code renderView()}模板方法执行
	 * 
	 * 
