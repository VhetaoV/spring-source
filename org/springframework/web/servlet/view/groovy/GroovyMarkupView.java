 ...
 * 
 *  Spring的Groovy Markup Template支持需要Groovy 231和更高版本
 * 
 * 
 * @author Brian Clozel
...
public class GroovyMarkupView extends AbstractTemplateView {
	 ...
	 * <p>
	 * 将MarkupTemplateEngine设置为在此视图中使用<p>如果未设置,则会通过在Web应用程序上下文中查找单个{@link GroovyMarkupConfig} bean并使用它来获取配置的
	 * {@code MarkupTemplateEngine}实例来自动检测引擎。
	 * 
	 * 
	 ...
	 * <p>
	 *  在启动时调用如果没有手动设置{@link #setTemplateEngine(MarkupTemplateEngine)templateEngine},此方法将按类型查找{@link GroovyMarkupConfig}
	 *  bean,并使用它来获取Groovy标记模板引擎。
	 * 
	 * 
	 * @see GroovyMarkupConfig
	 ...
	 * <p>
	 *  通过ApplicationContext自动检测MarkupTemplateEngine如果MarkupTemplateEngine尚未手动配置,则调用
	 * 
	 */
	 ...
	 * <p>
	 *  返回由给定视图URL配置的Groovy标记模板引擎编译的模板
	 */
