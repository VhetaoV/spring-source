 ...
 *
 * <p>
 *  VelocityLayoutView模拟Velocity的VelocityLayoutServlet提供的功能,以简化不同模板的页面组合
 * 
 * 应将{@code url}属性设置为视图的内容模板,并将布局模板位置指定为{@code layoutUrl}属性视图可以通过设置相应的键来覆盖配置的布局模板位置(默认为"布局")
 * 
 *  <p>当渲染视图时,VelocityContext首先与内容模板(由{@code url}属性指定)合并),然后与布局模板合并以产生最终输出
 * 
 *  <p>布局模板可以通过VelocityContext变量(默认为"screen_content")包含屏幕内容。在运行时,此变量将包含呈现的内容模板
 * 
 * 
...
public class VelocityLayoutView extends VelocityToolboxView {
	 ...
	 * <p>
	 *  默认{@link #setLayoutUrl(String)layout url}
	 * 
	 */
	 ...
	 * <p>
	 * 默认的{@link #setLayoutKey(String)布局键}
	 * 
	 */
	 ...
	 * <p>
	 *  默认{@link #setScreenContentKey(String)屏幕内容密钥}
	 * 
	 */
	 ...
	 * <p>
	 *  设置布局模板使用默认为{@link #DEFAULT_LAYOUT_URL"layoutvm"}
	 * 
	 * 
	 ...
	 *  <p>默认键为{@link #DEFAULT_LAYOUT_KEY"layout"},如上所示。
	 * 
	 * 
	 ...
	 * <p>
	 * 设置将保存布局模板内屏幕内容的上下文键的名称此键必须存在于要渲染的当前屏幕的布局模板中。
	 * <p>默认值为{@link #DEFAULT_SCREEN_CONTENT_KEY"screen_content"}：在VTL中访问{@code $ screen_content}。
	 * 
	 * 
	 ...
	 * <p>
	 *  覆盖{@code VelocityViewcheckTemplate()}以额外检查布局模板和屏幕内容模板是否可以加载注意,在渲染屏幕内容期间,可以更改布局模板,这可能会使任何早期检查无效
	 * 
	 */
	@Override
	 ...
	 * <p>
	 * 覆盖正常的渲染过程,以便预处理上下文,将其与屏幕模板合并为单个值(由screenContentKey的值标识)然后将布局模板与超类中的修改的上下文合并
	 * 
	 */
	@Override
