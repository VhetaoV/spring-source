 ...
 *
 * <p>
 *  使用AcroForm应用程序特定的视图类对现有文档进行操作的PDF视图的抽象超类将扩展此类以将PDF表单与模型数据合并
 * 
 * 感谢Bryant Larsen的建议和原型！
 * 
 * 
...
public abstract class AbstractPdfStamperView extends AbstractUrlBasedView {
	 ...
	 * <p>
	 *  将原始PDF资源读入iText PdfReader <p>默认实现将指定的"url"属性解析为ApplicationContext资源
	 * 
	 * 
	 ...
	 * <p>
	 *  子类必须实现这种方法来将PDF表单与给定的模型数据进行合并<p>这是您可以在AcroForm上设置值的一个例子在此级别可以做的事情是：
	 * <pre class="code">
	 *  //从文档获取表单AcroFields form = stampergetAcroFields();
	 * 
	 *  //在formsetField("field1","value1")的表单上设置一些值; formsetField("field2","Vvlue2");
	 * 
