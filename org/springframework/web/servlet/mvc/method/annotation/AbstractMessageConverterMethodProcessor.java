 ...
 *
 * <p>
 *  扩展{@link AbstractMessageConverterMethodArgumentResolver},具有通过使用{@link HttpMessageConverter}写入响应来处理方法
 * 返回值的功能。
 * 
 * 
...
public abstract class AbstractMessageConverterMethodProcessor extends AbstractMessageConverterMethodArgumentResolver
	 ...
	 * <p>
	 * 具有转换器和ContentNegotiationManager列表的构造方法
	 * 
	 */
	protected AbstractMessageConverterMethodProcessor(List<HttpMessageConverter<?>> converters,
	 ...
	 * <p>
	 *  具有转换器和ContentNegotiationManager列表的构造方法以及请求/响应体建议实例
	 * 
	 */
	protected AbstractMessageConverterMethodProcessor(List<HttpMessageConverter<?>> converters,
	 ...
	 * <p>
	 *  从给定的{@link NativeWebRequest}中创建一个新的{@link HttpOutputMessage}
	 * 
	 * 
	 ...
	 * <p>
	 *  将给定的返回值写入给定的Web请求代理{@link #writeWithMessageConverters(Object,MethodParameter,ServletServerHttpRequest,ServletServerHttpResponse)}
	 * 。
	 * 
	 */
	 ...
	 * <p>
	 *  返回要写入响应的值的类型通常,这是通过getClass对值的简单检查,但是如果该值为空,那么需要检查返回类型,包括通用类型确定(例如{@code ResponseEntity < T>})
	 * 
	 */
	 ...
	 * <p>
	 * 返回{@code returnType}的通用类型(或者是{@link HttpEntity})的嵌套类型的通用类型)
	 * 
	 */
	 ...
	 *  返回可以生成的媒体类型：
	 * <ul>
	 *  <li>请求映射中指定的可生产的媒体类型,或<li>可以编写特定返回值的已配置转换器的媒体类型,或<li> {@ link MediaType#ALL}
	 * </ul>
	 * 
	 * @since 4.2
	 */
	 ...
	 * RFD exploits.
	 * <p>
	 *  检查路径是否有文件扩展名,以及该扩展名是否已经被列为{@link #WHITELISTED_EXTENSIONS whitelisted}或明确{@link ContentNegotiationManager#getAllFileExtensions())注册}
	 * 如果不是,并且该状态在2xx范围内,则"Content-Disposition添加了带有安全附件文件名("ftxt")的头文件以防止RFD漏洞利用。
	 */
