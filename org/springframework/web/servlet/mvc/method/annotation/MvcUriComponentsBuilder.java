 ...
 *
 * <p>
 *  通过指向Spring MVC控制器和{@code @RequestMapping}方法创建{@link orgspringframeworkwebutilUriComponentsBuilder}的实
 * 例。
 * 
 * <p>静态{@code fromXxx()}方法准备了通过调用{@link orgspringframeworkwebservletsupportServletUriComponentsBuilder#fromCurrentServletMapping()}
 * 确定的当前请求的链接。
 * 
 *  <p>静态{@code fromXxx(UriComponentsBuilder,)}方法可以在请求的上下文之外运行时被赋予baseUrl
 * 
 *  您也可以通过{@link #relativeTo(orgspringframeworkwebutilUriComponentsBuilder)}创建一个带有baseUrl的MvcUriComponen
 * tsBuilder实例,然后使用非静态{@code withXxx()}方法变体。
 * 
 * 
...
public class MvcUriComponentsBuilder {
	 ...
	 * <p>
	 *  Bean工厂中的{@link CompositeUriComponentsContributor}对象的知名名称
	 * 
	 */
	 ...
	 * <p>
	 * 使用基本URL创建此类的实例之后调用基于{@code withXxx(}}方法的一个实例将创建相对于给定基本URL的URL
	 * 
	 */
	 ...
	 * <p>
	 *  从控制器类的映射和当前请求信息(包括Servlet映射)创建一个{@link UriComponentsBuilder}如果控制器包含多个映射,则只使用第一个映射
	 * 
	 * 
	 ...
	 * <p>
	 *  接受表示基本URL的{@code UIDComponentsBuilder}的{@link #fromController(Class)}的替代方法当在处理请求的上下文中使用MvcUriCompone
	 * ntsBuilder或应用与当前请求不匹配的自定义baseUrl时,这很有用。
	 * 
	 * 
	 ...
	 * <p>
	 * 从控制器方法和方法参数值数组的映射创建一个{@link UriComponentsBuilder}该方法委托给{@link #fromMethod(Class,Method,Object)}
	 * 
	 * 
	 ...
	 * <p>
	 *  接受表示基本URL的{@code UriComponentsBuilder}的{@link #fromMethodName(Class,String,Object)}的替代方法在处理请求的上下文中使用
	 * MvcUriComponentsBuilder或应用不匹配的自定义baseUrl时,这很有用当前请求。
	 * 
	 * 
	 * @param builder the builder for the base URL; the builder will be cloned
	 * and therefore not modified and may be re-used for further calls.
	 ...
	 * </pre>
	 * <p>
	 *  通过调用"mock"控制器方法创建一个{@link UriComponentsBuilder}然后将控制器方法和提供的参数值委托给{@link #fromMethod(Class,Method,Object)}
	 *  <p>例如,给定这个控制器：。
	 * <pre class="code">
	 * @RequestMapping("/ people / {id} / addresses")class AddressController {
	 * 
	 *  @RequestMapping("/ {country}")public HttpEntity <Void> getAddressesForCountry(@PathVariable String c
	 * ountry){}。
	 * 
	 *  @RequestMapping(value ="/",method = RequestMethodPOST)public void addAddress(Address address){}}
	 * </pre>
	 *  可以创建一个UriComponentsBuilder：
	 * <pre class="code">
	 *  //静态导入"MvcUriComponentsBuilderon"的内联样式
	 * 
	 *  MvcUriComponentsBuilderfromMethodCall(on(AddressControllerclass)getAddressesForCountry("US"))buildAn
	 * dExpand(1);。
	 * 
	 *  //用于重复调用的更长的表单(和void控制器方法)
	 * 
	 * AddressController controller = MvcUriComponentsBuilderon(AddressControllerclass); controlleraddAddres
	 * s(NULL); builder = MvcUriComponentsBuilderfromMethodCall(controller); controlsergetAddressesForCountr
	 * y("US")builder = MvcUriComponentsBuilderfromMethodCall(controller);。
	 * </pre>
	 * 
	 ...
	 * request or to apply a custom baseUrl not matching the current request.
	 * <p>
	 *  接受表示基本URL的{@code UriComponentsBuilder}的{@link #fromMethodCall(Object)}的替代方法当在处理请求的上下文中使用MvcUriCompon
	 * entsBuilder或应用与当前请求不匹配的自定义baseUrl时,这是有用的。
	 * 
	 * 
	 * @param builder the builder for the base URL; the builder will be cloned
	 * and therefore not modified and may be re-used for further calls.
	 ...
	 * <p>
	 * 从Spring MVC控制器方法的请求映射的名称创建一个URL <p>配置的{@link orgspringframeworkwebservlethandlerHandlerMethodMappingNamingStrategy HandlerMethodMappingNamingStrategy}
	 * 确定启动时控制器方法请求映射的名称默认情况下,所有映射都将根据类名称,后跟"#"作为分隔符,然后是方法名称例如"PC#getPerson"用于名为PersonController的类与方法getPers
	 * on如果命名约定不产生唯一的结果,则可以通过{@code @RequestMapping}注释的名称属性<p>这主要用于视图渲染技术和EL表达式Spring URL标签库将此方法注册为名为"mvcUrl
	 * "的函数<p>例如,给定此控制器：。
	 * <pre class="code">
	 * @RequestMapping("/ people")class PersonController {
	 * 
	 *  @RequestMapping("/ {id}")public HttpEntity <Void> getPerson(@PathVariable String id){}
	 * 
	 *  }
	 * </pre>
	 * 
	 *  JSP可以如下准备一个控制器方法的URL：
	 * 
	 * <pre class="code">
	 * <%@ taglib uri="http://www.springframework.org/tags" prefix="s" %>
	 * 
	 *  &lt; a href ="$ {s：mvcUrl('PC#getPerson')arg(0,"123")build()}"&gt; Get Person&lt; / a&gt;
	 * </pre>
	 *  注意,没有必要指定所有参数只有准备URL所需的参数,主要是{@code @RequestParam}和{@code @PathVariable})
	 * 
	 * 
	 ...
	 * request or to apply a custom baseUrl not matching the current request.
	 * <p>
	 *  接受表示基本URL的{@code UriComponentsBuilder}的{@link #fromMappingName(String)}的替代方法当在处理请求的上下文中使用MvcUriCompo
	 * nentsBuilder或应用与当前请求不匹配的自定义baseUrl时,这很有用。
	 * 
	 * 
	 * @param builder the builder for the base URL; the builder will be cloned
	 * and therefore not modified and may be re-used for further calls.
	 ...
	 * can be {@code null}.
	 * <p>
	 * 从控制器方法的映射和方法参数值的数组中创建{@link UriComponentsBuilder}值的数组必须与控制器方法的签名相匹配{@code @RequestParam}和{@code @PathVariable}
	 * 的值被使用用于构建URI(通过{@link orgspringframeworkwebmethodsupportUriComponentsContributor UriComponentsContributor}
	 * 的实现),而剩余的参数值被忽略,可以是{@code null}。
	 * 
	 * 
	 ...
	 * current request.
	 * <p>
	 *  接受代表基本URL的{@code UriComponentsBuilder}的{@link #fromMethod(Class,Method,Object)}的替代方法当在处理请求的上下文中使用Mvc
	 * UriComponentsBuilder或应用不匹配的自定义baseUrl时,这很有用当前请求。
	 * 
	 * 
	 ...
	 * </pre>
	 * <p>
	 * 返回一个"mock"控制器实例当控制器上的{@code @RequestMapping}方法被调用时,提供的参数值将被记住,然后可以通过{@link #fromMethodCall()创建一个{@code UriComponentsBuilder}
	 *  Object)} <p>请注意,这是用于内联使用(带静态导入)的{@link #controller(Class)}的简写版本,例如：。
	 * <pre class="code">
	 *  MvcUriComponentsBuilderfromMethodCall(上(FooControllerclass)的getFoo(1))的构建();
	 * </pre>
	 * 
	 ...
	 * </pre>
	 * <p>
	 * 返回一个"mock"控制器实例当控制器上的{@code @RequestMapping}方法被调用时,提供的参数值被记住,然后可以使用结果通过{@link #fromMethodCall(Object)来创建{@code UriComponentsBuilder}
	 *  )} <p>这是一个较长的版本的{@link #on(Class)}需要与控制器方法返回void以及重复调用。
	 * <pre class="code">
	 *  FooController fooController = controller(FooControllerclass);
	 * 
	 *  fooControllersaveFoo(1,null); builder = MvcUriComponentsBuilderfromMethodCall(fooController);
	 * 
	 *  fooControllersaveFoo(2,null); builder = MvcUriComponentsBuilderfromMethodCall(fooController);
	 * </pre>
	 * 
	 * @param controllerType the target controller
	 */
	 ...
	 * <p>
	 *  用于通过调用{@link #relativeTo}创建的此类的实例使用的{@link #fromController(Class)}的替代方法
	 * 
	 * 
	 * @since 4.2
	 */
	 ...
	 * <p>
	 * 与{@link #relativeTo}创建的此类的实例一起使用的{@link #fromMethodName(Class,String,Object)}}的替代方法
	 * 
	 * 
	 * @since 4.2
	 */
	 ...
	 * <p>
	 *  与{@link #relativeTo}创建的此类的实例一起使用的{@link #fromMethodCall(Object)}的替代方法
	 * 
	 * 
	 * @since 4.2
	 */
	 ...
	 * of this class created via {@link #relativeTo}.
	 * <p>
	 *  与{@link #relativeTo}创建的此类实例一起使用的{@link #fromMappingName(String)}的替代方法
	 * 
	 * 
	 * @since 4.2
	 */
	 ...
	 * <p>
	 *  用于通过{@link #relativeTo}创建的此类的实例使用的{@link #fromMethod(Class,Method,Object)}的替代方法
	 * 
	 * 
	 * @since 4.2
	 */
