 ...
 *
 * <p>
 * 实现{@link orgspringframeworkwebservletHandlerMapping},其遵循从注册的{@link orgspringframeworkwebservletmvcController}
 *  bean的<i>类名称</i>以及{@code @Controller}注释的bean生成URL路径映射的简单约定。
 * 
 *  <p>对于简单的{@link orgspringframeworkwebservletmvcController}实现(那些处理单个请求类型)的实现,约定是使用{@code Class}的{@link ClassUtils#getShortName short name}
 * ,删除"Controller"后缀如果它存在并返回剩下的文本,则较低级,作为映射,带有一个前导{@code /}例如：。
 * <ul>
 *  <li> {@ code WelcomeController}  - > {@code / welcome *} </li> <li> {@ code HomeController}  - > {@code / home *}
 *  </li>。
 * </ul>
 * 
 * <p>对于{@code MultiActionController MultiActionControllerers}和{@code @Controller} bean,注册了类似的映射,但所有子路径都
 * 使用尾随的通配符模式{@code / *}进行注册。
 * 例如：。
 * <ul>
 *  <li> {@ code WelcomeController}  - > {@code / welcome},{@code / welcome / *} </li> <li> {@ code CatalogController}
 *   - > {@code / catalog},{@code /目录/ *} </LI>。
 * </ul>
 * 
 *  <p>对于{@code MultiActionController},将此映射策略与{@link orgspringframeworkwebservletmvcmultiactionInternalPathMethodNameResolver}
 * 结合使用通常很有用。
 * 
 *  感谢Warren Oliver提出了在Spring 25中添加的"caseSensitive","pathPrefix"和"basePackage"属性
 * 
 * 
 * @author Rob Harrop
...
public class ControllerClassNameHandlerMapping extends AbstractControllerUrlHandlerMapping {
	 ...
	 * e.g. turning the class name "BuyForm" into "buyform".
	 * <p>
	 *  设置是否对生成的路径应用区分大小写,例如将类名称"BuyForm"转换为"buyForm"<p>默认为"false",使用纯​​小写路径,例如将类名称"BuyForm"转换为"buyform"
	 * 
	 */
	 ...
	 * <p>
	 *  指定前缀到从控制器名称生成的路径<p>默认值为纯斜线("/")可以指定像"/ mymodule"这样的路径,以使控制器路径映射以该路径为前缀,例如" / mymodule / buyform"而不是"
	 * / buyform"的类名称"BuyForm"。
	 * 
	 */
	 ...
	 * <p>
	 * 设置用于生成路径映射的基本包,包括此包下面的所有子包作为路径元素<p>默认为{@code null},使用生成路径的短类名称,控制器的包不在path指定基本包,如"commycompanymyapp",
	 * 以将该基本包中的子包包含为路径元素,例如为类名"commycompanymyappmymoduleBuyForm"生成路径"/ mymodule / buyform"。
	 * 子包层次结构表示为各个路径元素,例如"/ mymodule / mysubmodule / buyform"为类名"commycompanymyappmymodulemysubmoduleBuyForm
	 * "。
	 * 
	 */
	 ...
	 * <p>
	 * 生成给定控制器类的实际URL路径<p>子类可以选择自定义通过覆盖此方法生成的路径
	 * 
	 * 
	 ...
	 * <p>
	 *  } else {return new String [] {pathMappingtoString()+"*"}; }}
	 * 
	 * 
	 * @param beanClass the controller bean class to generate a mapping for
