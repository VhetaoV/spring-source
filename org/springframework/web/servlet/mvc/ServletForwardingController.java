 ...
 *
 * <p>
 * Spring Controller实现转发到一个指定的servlet,即webxml中的"servlet-name",而不是一个URL路径映射目标servlet在webxml中首先不需要"servlet
 * 映射"："servlet"声明就足够了。
 * 
 *  <p>可以通过Spring调度基础架构调用现有的servlet,例如将Spring HandlerInterceptors应用到其请求中即使在不支持Servlet过滤器的最小Servlet容器中也可以
 * 运行。
 * 
 *  例如：</b> webxml,将所有"/ myservlet"请求映射到一个Spring调度程序中也定义了一个自定义的"myServlet",但是<i>没有</i> servlet映射
 * 
 * <pre class="code">
 * &LT; servlet的&GT; &LT; servlet的名称&gt;&myServlet LT; / servlet的名称&gt; &LT; servlet的类&GT; mypackageTest
 * Servlet&LT; / servlet的类&GT; &LT; / servlet的&GT;。
 * 
 *  &LT; servlet的&GT; &LT; servlet的名称&gt;&myDispatcher LT; / servlet的名称&gt; &LT; servlet的类&GT; orgspring
 * frameworkwebservletDispatcherServlet&LT; / servlet的类&GT; &LT; / servlet的&GT;。
 * 
 *  &LT; servlet的映射&GT; &LT; servlet的名称&gt;&myDispatcher LT; / servlet的名称&gt; &LT; URL模式&GT; / myservlet
 * &LT; / url-pattern的&GT; &LT; / servlet的映射&GT; </PRE>。
 * 
 *  <b>示例：</b> myDispatcher-servletxml反过来将"/ myservlet"转发到您的servlet(由servlet名称标识)所有这些请求将通过配置的HandlerInte
 * 
...
public class ServletForwardingController extends AbstractController implements BeanNameAware {
	 ...
	 * <p>
	 * rceptor链(例如OpenSessionInViewInterceptor)从servlet的角度一切都照常工作。
	 * 
	 * <pre class="code">
	 * &lt; bean id ="urlMapping"class ="orgspringframeworkwebservlethandlerSimpleUrlHandlerMapping"&gt; &lt
	 * ; property name ="interceptors"&gt; &LT;列表&gt; &lt; ref bean ="openSessionInViewInterceptor"/&gt; &LT
	 * ; /列表&gt; &LT; /性&gt; &lt; property name ="mappings"&gt; &LT;道具&GT; &lt; prop key ="/ myservlet"&gt; 
	 * myServletForwardingController&lt; / prop&gt; &LT; /道具&GT; &LT; /性&gt; &LT; /豆腐&GT;。
	 * 
	 *  &lt; bean id ="myServletForwardingController"class ="orgspringframeworkwebservletmvcServletForwardin
	 * gController"&gt; &lt; property name ="servletName"&gt;&lt; value&gt; myServlet&lt; / value&gt;&lt; / 
	 */
