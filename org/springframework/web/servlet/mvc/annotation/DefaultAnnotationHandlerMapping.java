 ...
 *
 * <p>
 *  根据类型或方法级别通过{@link RequestMapping}注释表示的HTTP路径来实现映射处理程序的{@link orgspringframeworkwebservletHandlerMapping}
 * 接口。
 * 
 * 注意：</b>如果您在DispatcherServlet上下文中定义了自定义HandlerMapping bean,则需要显式添加DefaultAnnotationHandlerMapping bean
 * ,因为自定义HandlerMapping bean将被替换默认映射策略定义DefaultAnnotationHandlerMapping还允许注册自定义拦截器：。
 * 
 * <pre class="code">
 *  &lt; bean class ="orgspringframeworkwebservletmvcannotationDefaultAnnotationHandlerMapping"&gt; &lt;
 *  property name ="interceptors"&gt; &LT; /性&gt; &LT; /豆腐&GT; </PRE>。
 * 
 * 注释控制器通常在类型级别标有{@link控制器}构造型别在类型级别应用{@link RequestMapping}时,这并不是绝对必需的(因为这样的处理程序通常会实现{@link orgspringframeworkwebservletmvcController}
 * 接口)如果在类型级别不存在{@link RequestMapping},则需要{@link Controller}在方法级检测{@link RequestMapping}注释。
 * 
 * <p> <b>注意：</b>只允许方法级映射来缩小在类级别(如果有的话)表达的映射,HTTP路径需要唯一映射到特定的处理程序bean,只允许任何给定的HTTP路径映射到一个特定的处理程序bean(不分布
 * 在多个处理程序bean中)强烈建议将相关的处理程序方法共同定位到同一个bean中。
 * 
 *  <p> {@link AnnotationMethodHandlerAdapter}负责处理类型级别的此HandlerMapping For {@link RequestMapping}映射的注释处理
 * 程序方法,特定的HandlerAdapter(例如{@link orgspringframeworkwebservletmvcSimpleControllerHandlerAdapter} apply。
 * 
 * 
...
public class DefaultAnnotationHandlerMapping extends AbstractDetectingUrlHandlerMapping {
	 ...
	 * <p>
	 * 设置是否使用默认后缀模式注册路径：即"/ users"是否应该注册为"/ users","/ users /"也是<p>默认为"true"如果您打算关闭此约定要严格解释您的{@code @RequestMapping}
	 * 路径<p>请注意,包含"xxx"后缀或"/"结尾的路径在任何情况下都不会使用默认后缀模式进行转换。
	 * 
	 */
	 ...
	 * <p>
	 *  检查在处理程序类和其任何方法上是否存在{@link orgspringframeworkwebbindannotationRequestMapping}注释
	 * 
	 */
	@Override
	 ...
	 * <p>
	 *  根据当前请求验证给定的类型级映射元数据,检查HTTP请求方法和参数条件
	 * 
