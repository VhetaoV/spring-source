 ...
 *
 * <p>
 *  用于从{@code spring：bind}标签中的表单控制器和其他对象转换参考数据值的标签(或Spring表单标签库中的数据绑定表单元素标签)
 * 
 * BindTag有一个PropertyEditor,它用于将bean的属性转换为String,可用于HTML表单此标记使用该PropertyEditor来转换传递到此标签中的对象
 * 
 * 
...
public class TransformTag extends HtmlEscapingAwareTag {
	 ...
	 * <p>
	 *  设置要转换的值,使用封闭的BindTag中的相应PropertyEditor <p>该值可以是要转换的简单值(JSP或JSP表达式中的硬编码字符串值)或JSP EL表达式评估(转换表达式的结果)
	 * 
	 */
	 ...
	 * <p>
	 *  设置在其中显示包含转换结果的变量的PageContext属性名称
	 * 
	 * 
	 * @see #setScope
