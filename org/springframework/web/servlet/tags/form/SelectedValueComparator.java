 ...
 *
 * <p>
 * 用于测试候选值是否匹配{@link BindStatus#getValue数据绑定值}的实用程序类。
 * 试图通过一些途径来证明比较,以处理诸如实例不等式,基于逻辑(基于字符串表示的)平等和基于{@link PropertyEditor}的比较。
 * 
 *  <p>完全支持比较阵列{@link集合集合}和{@link地图地图}
 * 
 * <p> <h1> <a name=\"equality-contract\">平等合同</a> </h1>对于单值对象,首先使用标准的{@link对象#等于Java等级}测试对等体。
 * 因此,用户代码应该努力实现{@link Object#equals}来加快比较过程如果{@link Object#equals}返回{@code false},那么尝试在{@link #exhaustiveCompare穷举比较}
 * 中进行目标要<strong>证明</strong>平等而不是反驳。
 * <p> <h1> <a name=\"equality-contract\">平等合同</a> </h1>对于单值对象,首先使用标准的{@link对象#等于Java等级}测试对等体。
 * 
 * 
 * @author Rob Harrop
...
abstract class SelectedValueComparator {
	 ...
	 * <p>
	 *  <p>接下来,尝试比较候选和绑定值的{@code String}表示。这可能导致{@code true}在一些情况下由于两个值都将被表示为{ @code Strings}向用户显示
	 * 
	 * <p>接下来,如果候选值是{@code String},则尝试将绑定值与将相应的{@link PropertyEditor}应用于候选者的结果进行比较。
	 * 此比较可以执行两次,一次针对直接{@code String}实例,然后针对{@code String}表示,如果第一个比较导致{@code false}。
	 * 
	 */
