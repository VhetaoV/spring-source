 ...
 *
 * <p>
 *  用于格式化用于通过表单标签呈现的值的Package-visible帮助器类支持两种格式的格式：plain和{@link PropertyEditor} -aware
 * 
 * 普通格式化只会阻止字符串"{@code null}"出现,用空字符串替换它,并根据需要添加HTML转义
 * 
 *  <p> {@ link PropertyEditor} -aware格式化将尝试使用提供的{@link PropertyEditor}在应用纯格式化的默认规则之前呈现任何非String值
 * 
 * 
 * @author Rob Harrop
...
abstract class ValueFormatter {
	 ...
	 * <p>
	 *  构建提供的{@code Object}的显示值,根据需要转义HTML此版本<strong>不</strong> {@link PropertyEditor} -aware
	 * 
	 * 
	 ...
	 * <p>
	 *  构建提供的{@code Object}的显示值,根据需要转义HTML如果提供的值不是{@link String},并且提供的{@link PropertyEditor}不为null,则使用{@link PropertyEditor}
	 * 获取显示值。
	 * 
