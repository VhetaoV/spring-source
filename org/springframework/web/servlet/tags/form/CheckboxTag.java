 ...
 *
 * <p>
 *  数据绑定感知的JSP标签用于使用"{@code checkbox}"的"{@code type}"呈现HTML"{@code input}"元素
 * 
 * <p>可以根据{@link #getValue绑定值的类型来使用三种不同的方法之一
 * 
 * <h3>接近一个</h3>当绑定值类型为{@link Boolean}时,如果绑定值为{@code true},则'{@code input(checkbox)}'被标记为"checked" "{@code value}
 * 
 * @author Rob Harrop
