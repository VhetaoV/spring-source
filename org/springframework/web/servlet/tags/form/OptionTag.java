 ...
 *
 * <p>
 *  用于呈现HTML"{@code选项}"标签的JSP标签
 * 
 *  <p> <b>必须嵌套在{@link SelectTag} </b>内
 * 
 * 如果{@link #setValue值}与绑定到外部{@link SelectTag}的值匹配,则可以通过将"{@code选项}"标记为"已选择"来完全支持数据绑定
 * 
 *  <p> {@link #setValue value}属性是必需的,对应于渲染的"{@code选项}"的"{@code value}"属性
 * 
 *  <p>可以指定一个可选的{@link #setLabel label}属性,其值对应于渲染的"{@code选项}"标签的内部文本。
 * 如果未指定{@link #setLabel label},则{渲染内部文本时将使用@link #setValue value}属性。
 * 
 * 
 * @author Rob Harrop
...
public class OptionTag extends AbstractHtmlElementBodyTag implements BodyTag {
	 ...
	 * <p>
	 *  设置呈现的HTML {@code <option>}标签的"值"属性
	 * 
	 */
	 ...
	 * <p>
	 *  设置呈现的HTML {@code <option>}标签的文本正文<p>可以是运行时表达式
	 * 
	 */
	 ...
	 * argument is used.
	 * <p>
	 * 返回此'{@code选项}'元素的标签值如果设置了{@link #setLabel label}属性,则使用该属性的解析值,否则{@code resolvedValue}参数的值为用过的
	 */
