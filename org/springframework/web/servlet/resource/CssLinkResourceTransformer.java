 ...
 *
 * <p>
 *  {@link ResourceTransformer}实现,修改CSS文件中的链接,以匹配应暴露给客户端的公共URL路径(例如,在URL中插入基于MD5内容的哈希值)
 * 
 * <p>实现在CSS {@code @import}语句和CSS {@code url()}函数中查找链接所有链接然后通过{@link ResourceResolverChain}传递,并相对于包含的位置
 * 进行解析CSS文件如果成功解决,链接被修改,否则原始链接被保留。
 * 
 * 
...
public class CssLinkResourceTransformer extends ResourceTransformerSupport {
	...
	protected static abstract class AbstractCssLinkParser implements CssLinkParser {
		 ...
		 * <p>
		 *  在关键字匹配之后调用,除去空格后,下一个字符既不是单引号也不是双引号
		 */
