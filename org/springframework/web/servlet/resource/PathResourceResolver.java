 ...
 *
 * <p>
 *  一个简单的{@code ResourceResolver},尝试在与请求路径匹配的给定位置下查找资源
 * 
 * <p>此解析器未委派给{@code ResourceResolverChain},预计将在解析器链中最后配置
 * 
 * 
...
public class PathResourceResolver extends AbstractResourceResolver {
	 ...
	 * <p>
	 * 默认情况下,当找到资源时,比较解析资源的路径,以确保其位于被发现的输入位置。
	 * 但有时可能不是这样,例如,当{@link orgspringframeworkwebservletresourceCssLinkResourceTransformer}解析其包含的链接的公共URL时,C
	 * SS文件是位置,正在解析的资源是位于相邻或父目录中的css文件,图像,字体和其他位置<p>此属性允许配置资源必须位置的完整列表,以便如果资源为不在相对于它找到的位置下,也可以检查此列表<p>默认情况下,
	 * {@link ResourceHttpRequestHandler}初始化此属性以匹配其位置列表。
	 * 默认情况下,当找到资源时,比较解析资源的路径,以确保其位于被发现的输入位置。
	 * 
	 * 
	 ...
	 * <p>
	 * 查找给定位置下的资源<p>默认实现检查相对于位置的给定路径是否存在可读{@code Resource}
	 * 
	 * 
