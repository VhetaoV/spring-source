 ...
 *
 * <p>
 *  始终返回固定的默认语言环境和可选时区的{@link orgspringframeworkwebservletLocaleResolver}实现默认值是当前JVM的默认语言环境
 * 
 * <p>注意：不支持{@code setLocale(Context)},因为固定的区域设置和时区不能更改
 * 
 * 
...
public class FixedLocaleResolver extends AbstractLocaleContextResolver {
	 ...
	 * <p>
	 *  创建默认的FixedLocaleResolver,将已配置的默认语言环境(或JVM的默认语言环境)暴露为备用
	 * 
	 * 
	 * @see #setDefaultLocale
	 * @see #setDefaultTimeZone
	 ...
	 * <p>
	 *  创建一个公开给定区域设置的FixedLocaleResolver
	 * 
	 * 
	 ...
	 * <p>
	 *  创建一个公开给定的区域设置和时区的FixedLocaleResolver
	 * 
	 * @param locale the locale to expose
