 ...
 *
 * <p>
 *  将HTTP {@code GET}请求映射到特定处理程序方法的注释
 * 
 * <p>具体来说,{@code @GetMapping}是一个组成的<em>注释</em>,作为{@code @RequestMapping(method = RequestMethodGET)}的快捷方
 * 式。
 * 
 *  <h5> {@code @GetMapping}&amp; {@code @RequestMapping} </h5> <p> {@ code @GetMapping}不支持{@code @RequestMapping}
 * 的{@link RequestMapping#耗用}属性。
 * 
 * 
 * @author Sam Brannen
 * @since 4.3
