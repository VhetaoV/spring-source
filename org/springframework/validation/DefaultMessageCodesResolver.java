 ...
 *
 * <p>
 *  默认实现{@link MessageCodesResolver}界面
 * 
 * <p>将按照以下顺序(当使用{@link Format#PREFIX_ERROR_CODE前缀} {@link #setMessageCodeFormatter(MessageCodeFormatter)格式化程序)时,为对象错误创建两个消息代码))：。
 * <ul>
 *  <li> 1：代码+""+对象名称<li> 2：代码
 * </ul>
 * 
 *  <p>将按照以下顺序为字段规范创建四个消息代码：
 * <ul>
 *  <li> 1：代码+""+对象名称+""+字段<li> 2：代码+""+字段<li> 3：代码+""+字段类型<li> 4：代码
 * </ul>
 * 
 *  例如,在代码"typeMismatch"的情况下,对象名称"user",字段"age"：
 * <ul>
 *  尝试"typeMismatchuserage"<li> 2尝试"typeMismatchage"<li> 3尝试"typeMismatchint"<li> 4尝试"typeMismatch"
 * </ul>
 * 
 * <p>因此,可以利用此分辨率算法来显示绑定错误(例如"required"和"typeMismatch")的特定消息：
 * <ul>
 *  <li>在对象+字段级别("age"字段,但仅限于"user"); <li>在字段级别(所有"年龄"字段,无论哪个对象名称); <li>或在一般级别(所有字段,任何对象)
 * </ul>
 * 
 *  <p>如果是数组,{@link列表}或{@link javautilMap}属性,则会生成特定元素和整个集合的两个代码。假设对象"user"中的数组"groups"的字段"name" ：
 * <ul>
 * 尝试"typeMismatchusergroups [0] name"<li> 2 try"typeMismatchusergroupsname"<li> 3 try"typeMismatchgroup
 * s [0] name"<li> 4 try"typeMismatchgroupsname"<li> 5 try"typeMismatchname"<li > 6尝试"typeMismatchjavala
 * ngString"<li> 7尝试"typeMismatch"。
 * </ul>
 * 
 *  <p>默认情况下,{@code errorCode}将被放置在构造的消息字符串的开头。
 * {@link #setMessageCodeFormatter(MessageCodeFormatter)messageCodeFormatter}属性可用于指定替代连接{@link MessageCodeFormatter format}
 * 。
 *  <p>默认情况下,{@code errorCode}将被放置在构造的消息字符串的开头。
 * 
 *  <p>为了将所有代码分组到资源束中的特定类别中,例如"validationtypeMismatchname"而不是默认的"typeMismatchname",请考虑指定要应用的{@link #setPrefix前缀}
 * 。
 * 
...
public class DefaultMessageCodesResolver implements MessageCodesResolver, Serializable {
	 ...
	 * <p>
	 *  指定要应用于由此解析器构建的任何代码的前缀<p>默认值为none指定例如"验证"以获取错误代码,如"validationtypeMismatchname"
	 * 
	 * 
	 * @since 3.2
	 * @see Format
	 ...
	 * <p>
	 *  指定此解析器生成的消息代码的格式<p>默认值为{@link Format#PREFIX_ERROR_CODE}
	 * 
	 */
	 ...
	 * <p>
	 *  返回要应用于由此解析器构建的任何代码的前缀<p>如果没有前缀,返回一个空字符串
	 * 
	 * 
	 ...
	 * <p>
	 * 构建给定代码和字段的代码列表：对象/字段特定代码,字段特定代码,普通错误代码<p>数组,列表和地图都针对特定元素和整个集合进行解析<p>有关生成的代码的详细信息,请参阅{@link DefaultMessageCodesResolver类级别Javadoc}
	 * 。
	 * 
	 */
	 ...
	 * <p>
	 *  将提供的{@code字段}的键控和非密钥条目添加到提供的字段列表
	 * 
	 * 
	 ...
	 *
	 * <p>
	 *  后处理由此解析器构建的给定消息代码<p>默认实现应用指定的前缀(如果有)
	 * 
	 * 
	 * @author Phillip Webb
	 * @author Chris Beams
	...
	public static enum Format implements MessageCodeFormatter {
		 ...
		 * <p>
		 *  在生成的消息代码的开始处将错误代码前缀,例如：{@code errorCode +""+ object name +""+ field}
		 * 
		 */
		POSTFIX_ERROR_CODE {
			@Override
		 ...
		 * <p>
		 * 在生成的消息代码的末尾Postfix错误代码,例如：{@code对象名称+""+字段+""+ errorCode}
		 * 
		 */
