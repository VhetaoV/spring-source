 ...
 *
 * <p>
 *  处理{@code DataBinder}缺少字段错误的策略,以及将{@code PropertyAccessException}翻译成{@code FieldError}
 * 
 * <p>错误处理器是可插拔的,因此如果您希望为典型需求提供默认实现,则可以不同地对待错误
 * 
 *  注意：从Spring 20开始,该接口在给定的BindingResult上运行,与任何绑定策略(bean属性,直接字段访问等)兼容。
 * 它仍然可以接收BindException作为参数(因为BindException实现了BindingResult接口),但不再直接运行。
 * 
 * 
...
public interface BindingErrorProcessor {
	 ...
	 * <p>
	 *  将丢失的字段错误应用于给定的BindException <p>通常,为缺少的必填字段创建字段错误
	 * 
	 * 
