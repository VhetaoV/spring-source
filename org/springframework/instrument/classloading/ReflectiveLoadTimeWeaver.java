 ...
 *
 * <p>
 * {@link LoadTimeWeaver},它使用反射来委托具有众所周知的变换钩子的底层ClassLoader底层ClassLoader将支持以下编织方法(如{@link LoadTimeWeaver}
 * 界面中定义的)：。
 * <ul>
 *  <li> {@ code public void addTransformer(javalanginstrumentClassFileTransformer)}：用于在此ClassLoader上注册给
 * 定的ClassFileTransformer <li> {@ code public ClassLoader getThrowawayClassLoader()}：用于为此ClassLoader获取一次
 * 性类加载器(可选; ReflectiveLoadTimeWeaver将会下降如果该方法不可用,返回到SimpleThrowawayClassLoader)。
 * </ul>
 * 
 * <p>请注意,上述方法<i>必须</i>驻留在可公开访问的类中,尽管类本身不必对应用程序的类加载器可见
 * 
 *  <p>当底层的ClassLoader实现被加载到不同的类加载器本身(例如应用程序服务器的类加载器,Web应用程序不可见)时,此LoadTimeWeaver的反射性质特别有用。
 * 
 * @author Costin Leau
...
public class ReflectiveLoadTimeWeaver implements LoadTimeWeaver {
	 ...
	 * <p>
	 * 在此之间没有直接的API依赖关系LoadTimeWeaver适配器和底层的ClassLoader,只是一个"松散"的方法契约。
	 * 
	 * <p>这是与Tomcat 50+的Spring的{@link orgspringframeworkinstrumentclassloadingtomcatTomcatInstrumentableClassLoader}
	 * 结合使用的LoadTimeWeaver以及Resin应用程序服务器版本31+。
	 * 
	 */
	 ...
	 * <p>
	 *  为当前的上下文类加载器创建一个新的ReflectiveLoadTimeWeaver,<i>需要支持所需的编织方法</i>
	 * 
	 * 
