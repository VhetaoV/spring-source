 ...
 *
 * <p>
 *  {@code HttpMessageConverter},使用<a href=\"https://developersgooglecom/protocol-buffers/\"> Google协议缓冲
 * 区</a>读取和写入{@link comgoogleprotobufMessage}。
 * 
 * <p>默认情况下,它支持{@code"application / x-protobuf"},{@code"text / plain"},{@code"application / json"},{@code"application / xml"}
 * ,同时也写{@code"text / html"}。
 * 
 *  <p>要生成{@code Message} Java类,需要安装{@code protoc}二进制文件
 * 
 *  <p>自Spring 43起,需要Protobuf 26和Protobuf Java Format 14
 * 
 * 
 * @author Alex Antonov
 * @author Brian Clozel
...
public class ProtobufHttpMessageConverter extends AbstractHttpMessageConverter<Message> {
	 ...
	 * <p>
	 *  使用允许注册消息扩展名的{@link ExtensionRegistryInitializer}构造新的实例
	 * 
	 */
	 ...
	 * <p>
	 *  此方法覆盖父实现,因为此HttpMessageConverter还可以生成{@code MediaTypeHTML"text / html"} ContentType
	 * 
	 */
	@Override
	 ...
	 * <p>
	 * 在使用内容类型"application / x-protobuf"的消息进行响应时,设置"X-Protobuf- *"HTTP标头<p> <b>注意：</b> <code> outputMessageg
	 * etBody()</code>不应该之前已经被调用过,因为它写入HTTP头(使它们只读)</p>。
	 * 
	 */
