 ...
 *
 * <p>
 *  {@link AbstractMessagingTemplate}的扩展,添加了将消息发送到由以下接口定义的可解析目标名称的操作：
 * <ul>
 * <li> {@ link DestinationResolvingMessageSendingOperations} </li> <li> {@ link DestinationResolvingMessageReceivingOperations}
 *  </li> <li> {@ link DestinationResolvingMessageRequestReplyOperations} </li>。
 * </ul>
 * 
 * 
 * @author Mark Fisher
...
public abstract class AbstractDestinationResolvingMessagingTemplate<D> extends AbstractMessagingTemplate<D>
	 ...
	 * <p>
	 *  配置{@link DestinationResolver}用于将String目标名称解析为{@code <D>}类型的实际目的地<p>此字段没有默认设置如果未配置,则需要解析目标名称的方法将提高{@link IllegalArgumentException}
	 * 。
	 * 
	 * 
