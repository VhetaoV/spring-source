 ...
 *
 * <p>
 * 代理目标DestinationResolver的{@link DestinationResolver}实现,缓存其{@link #resolveDestination}结果如果目标解析过程昂贵(例如,目
 * 标必须通过外部系统解决)并且解析结果,则此缓存特别有用还是稳定的。
 * 
 * 
 * @author Agim Emruli
...
public class CachingDestinationResolverProxy<D> implements DestinationResolver<D>, InitializingBean {
	 ...
	 * <p>
	 *  创建一个新的CachingDestinationResolverProxy,通过{@link #setTargetDestinationResolver} bean属性设置目标DestinationR
	 * esolver。
	 * 
	 */
	 ...
	 * <p>
	 *  使用给定的目标DestinationResolver创建一个新的CachingDestinationResolverProxy来实际解析目的地
	 * 
	 * 
	 ...
	 * <p>
	 *  解析和缓存目标,如果目标DestinationResolver实现成功解决
	 * 
