 ...
 *
 * <p>
 * 提供强类型getter和setter以及围绕特定类别标题的行为的基础(例如STOMP头)支持创建新标题,修改现有标题(仍可变),或复制和修改现有标题
 * 
 *  <p>方法{@link #getMessageHeaders()}提供对底层完全准备的{@link MessageHeaders}的访问,然后可以按原样使用(即不复制)创建单个消息,如下所示：
 * 
 * <pre class="code">
 *  MessageHeaderAccessor accessor = new MessageHeaderAccessor(); accessorsetHeader("foo","bar");消息消息= M
 * essageBuildercreateMessage("payload",accessorgetMessageHeaders());。
 * </pre>
 * 
 * 以上之后,默认情况下,{@code MessageHeaderAccessor}变为不可变但是,可以将其保留为相同线程中的进一步初始化,例如：
 * 
 * <pre class="code">
 *  MessageHeaderAccessor accessor = new MessageHeaderAccessor(); accessorsetHeader("foo","bar"); access
 * orsetLeaveMutable(真);消息消息= MessageBuildercreateMessage("payload",accessorgetMessageHeaders());。
 * 
 *  //以后在同一个线程
 * 
 *  MessageHeaderAccessor accessor = MessageHeaderAccessorgetAccessor(message); accessorsetHeader("bar",
 * "baz"); accessorsetImmutable();。
 * </pre>
 * 
 *  <p>方法{@link #toMap()}返回底层标题的副本它可以用于从同一个{@code MessageHeaderAccessor}实例准备多个消息：
 * <pre class="code">
 * MessageHeaderAccessor accessor = new MessageHeaderAccessor(); MessageBuilder builder = MessageBuilder
 * withPayload("payload")setHeaders(accessor);。
 * 
 *  accessorsetHeader("foo","bar1");消息message1 = builderbuild();
 * 
 *  accessorsetHeader("foo","bar2");消息message2 = builderbuild();
 * 
 *  accessorsetHeader("foo","bar3");消息message3 = builderbuild();
 * </pre>
 * 
 *  <p>但是请注意,使用上述样式,标头访问器是共享的,不能在以后重新获取。或者,也可以为每个消息创建一个{@code MessageHeaderAccessor}：
 * 
 * <pre class="code">
 *  MessageHeaderAccessor accessor1 = new MessageHeaderAccessor(); accessorset("foo","bar1");消息message1 
 * = MessageBuildercreateMessage("payload",accessor1getMessageHeaders());。
 * 
 * MessageHeaderAccessor accessor2 = new MessageHeaderAccessor(); accessorset("foo","bar2");消息message2 =
 *  MessageBuildercreateMessage("payload",accessor2getMessageHeaders());。
 * 
 *  MessageHeaderAccessor accessor3 = new MessageHeaderAccessor(); accessorset("foo","bar3");消息message3 
 * = MessageBuildercreateMessage("payload",accessor3getMessageHeaders());。
 * </pre>
 * 
 *  <p>请注意,上述示例旨在演示使用标头访问器的一般思想最有可能的用法是通过子类
 * 
 * 
...
public class MessageHeaderAccessor {
	 ...
	 * <p>
	 * 默认情况下,{@link #getMessageHeaders()}被调用,{@code"this"} {@code MessageHeaderAccessor}实例不能再用于修改底层消息头,返回的{@code MessageHeaders}
	 * 是不可变的>但是当这个设置为{@code true}时,返回的(底层的){@code MessageHeaders}实例保持不变。
	 * 为了进一步修改,继续使用相同的访问器实例,或通过以下方式重新获取：<br> {@link MessageHeaderAccessor#getAccessor(Message,Class)MessageHeaderAccessorgetAccessor(Message,Class)}
	 *  <p>修改完成后,使用{@link #setImmutable()}来防止进一步更改此机制的预期用例是单个消息中的消息初始化线程<p>默认设置为{@code false}。
	 * 
	 * 
	 * @since 4.1
	 */
	 ...
	 * <p>
	 * 默认情况下,{@link #getMessageHeaders()}被调用,{@code"this"} {@code MessageHeaderAccessor}实例不能再用于修改底层消息头。
	 * 但是如果{@link #setLeaveMutable(boolean)}是使用此方法时,必须明确指出何时不再修改{@code MessageHeaders}实例。
	 * 
	 * 
	 * @since 4.1
	 */
	 ...
	 * <p>
	 *  一个包私有机制,可以自动添加{@link orgspringframeworkmessagingMessageHeaders#TIMESTAMP}标头<​​p>默认情况下,此属性设置为{@code false}
	 * 。
	 * 
	 * 
	 ...
	 * <p>
	 * 用于配置IdGenerator策略使用<p>的package-private机制默认情况下,不设置此属性,在这种情况下,将使用{@link orgspringframeworkmessagingMessageHeaders}
	 * 中的默认IdGenerator。
	 * 
	 * 
	 * @see IdTimestampMessageHeaderInitializer
	 ...
	 * <p>
	 *  返回底层的{@code MessageHeaders}实例<p>除非{@link #setLeaveMutable(boolean)}设置为{@code true},否则此调用之后,标题是不可变的,此
	 * 访问者不能再修改它们<p>如果调用多次,此方法总是返回相同的{@code MessageHeaders}实例要获取底层标题的副本,请使用{@link #toMessageHeaders()}或{@link #toMap()}
	 * 。
	 * 
	 * 
	 * @since 4.1
	 ...
	 * <p>
	 * 返回底层头值的副本作为{@link MessageHeaders}对象<p>此方法可以被调用多次,修改之间每个新调用返回当前标题值的新副本
	 * 
	 * 
	 * @since 4.1
	 */
	 ...
	 * where each new call returns a fresh copy of the current header values.
	 * <p>
	 *  返回底层标题值的副本作为普通的{@link Map}对象<p>此方法可以被调用多次,修改之间每个新调用返回当前标题值的新副本
	 * 
	 */
	 ...
	 * <p>
	 *  设置给定标题名称的值<p>如果提供的值为{@code null},标题将被删除
	 * 
	 */
	 ...
	 * <p>
	 *  只有当标题名称尚未与某个值相关联时,才能为给定的标题名称设置值
	 * 
	 */
	 ...
	 * <p>
	 * 删除通过'headerPatterns'数组提供的所有标题<p>顾名思义,数组可能包含标题名称的简单匹配模式支持的模式样式有："xxx *","* xxx","* xxx *"和"xxx * YYY"。
	 * 
	 */
	 ...
	 * <p>
	 *  从提供的Map复制名称/值对<p>此操作将覆盖任何现有值使用{@link #copyHeadersIfAbsent(Map)}避免覆盖值
	 * 
	 */
	 ...
	 * <p>
	 *  从提供的地图复制名称/值对<p>此操作将<em>不</em>覆盖任何现有值
	 * 
	 */
	 ...
	 * <p>
	 * 返回用于创建给定{@code消息}的标头的原始{@code MessageHeaderAccessor},如果不可用则返回{@code null},或者其类型与所需类型不匹配<p>这是针对强烈预期存在访
	 * 问器的存在(如果没有被提供),或者将被创建。
	 * 
	 * 
	 ...
	 * <p>
	 *  {@link #getAccessor(orgspringframeworkmessagingMessage,Class)}与{@code MessageHeaders}实例而不是{@code消息} 
	 * <p>的变体这是针对完整消息可能尚未创建的情况。
	 * 
	 * 
	 ...
	 * <p>
	 * 为给定的消息返回一个可变的{@code MessageHeaderAccessor},尝试与用于创建消息头的访问器类型相匹配,或以{@code MessageHeaderAccessor}实例来包装消息
	 * 。
	 * 
