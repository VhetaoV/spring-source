 ...
 *
 * <p>
 *  用于创建Quartz {@link orgquartzCronTrigger}实例的Spring {@link FactoryBean},支持用于触发器配置的bean样式
 * 
 * 本身已经是一个JavaBean,但缺乏明智的默认值。此类使用Spring bean名称作为作业名称,Quartz默认组("DEFAULT")作为作业组,当前时间作为开始时间,无限期重复,如果没有规定
 * 
...
public class CronTriggerFactoryBean implements FactoryBean<CronTrigger>, BeanNameAware, InitializingBean {
	 ...
	 * <p>
	 * 通过{@link orgquartzCronTrigger}类中相应常量的名称设置失火指令。默认值为{@code MISFIRE_INSTRUCTION_SMART_POLICY}
	 * 
	 * 
