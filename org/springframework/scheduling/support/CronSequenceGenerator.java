 ...
 *
 * <p>
 *  用于<a href=\"http://wwwmanpagezcom/man/5/crontab/\"> Crontab模式</a>的日期序列生成器,允许客户端指定序列匹配的模式
 * 
 * <p>该模式是六个单独的空格分隔字段的列表：表示秒,分,小时,日,月,工作日可以给出英文名称的前三个字母的月和日名称
 * 
 *  <p>示例模式：
 * <ul>
 *  <li>"0 0 * * * *"=每天每小时的顶部</li> <li>"* / 10 * * * * *"=每10秒</li> <li>"0 0 8-10 * * *"=每天8,9和10点</li>
 *  <li>"0 * 6,19 * * *"=每天上午6:00和下午7:00 < / li> <li>"0 0/30 8-10 * * *"=每天8:00,8:30,9:00,9:30和10点</li> 
 * <li>"0 0 9-17 * * MON-FRI"=九至五个工作日小时</li> <li>"0 0 0 25 12?" =每个圣诞节在午夜</li>。
 * </ul>
 * 
 * 
 * @author Dave Syer
...
public class CronSequenceGenerator {
	 ...
	 * <p>
	 *  从提供的模式构造一个{@link CronSequenceGenerator},使用默认的{@link TimeZone}
	 * 
	 * 
	 ...
	 * <p>
	 * 从提供的模式构造一个{@link CronSequenceGenerator},使用指定的{@link TimeZone}
	 * 
	 * 
	 * @param expression a space-separated list of time fields
	 ...
	 * <p>
	 *  在与Cron模式匹配的顺序中获取下一个{@link Date},并在提供的值之后返回值将为整数秒,并将在输入值之后
	 * 
	 * 
	...
	public Date next(Date date) {
		...
		/*  1从整数开始(如有必要,舍入)
		/* 
		/*  2如果秒匹配移动,否则找到下一个匹配：21如果下一个匹配在下一分钟,然后滚动
		/* 
		/*  3如果分钟匹配移动,否则找到下一个匹配31如果下一个匹配是在下一个小时,然后滚动32重置秒钟,然后转到2
		/* 
		/* 4如果小时匹配移动,否则找到下一个匹配41如果下一个匹配是在第二天,然后滚动,42重置分钟和秒,并转到2
		/* 
		*/
	 ...
	 * <p>
	 *  确定指定的表达式是否表示有效的cron模式<p>具体来说,此方法验证表达式包含六个由单个空格分隔的字段
	 * 
