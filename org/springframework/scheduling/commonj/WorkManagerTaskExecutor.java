 ...
 *
 * <p>
 *  TaskExecutor实现委托给一个CommonJ WorkManager,实现{@link commonjworkWorkManager}接口,需要将其指定为引用或通过JNDI名称
 * 
 * <p> <b>这是在Spring上下文</b>中设置CommonJ WorkManager的中心便利类
 * 
 *  <p>还实现了CommonJ WorkManager接口本身,将所有调用委托给目标WorkManager因此,调用者可以选择是否要通过Spring TaskExecutor接口或CommonJ Wor
 * kManager接口与此执行程序通信。
 * 
 *  <p> CommonJ WorkManager通常将从服务器的管理控制台中定义的应用程序服务器的JNDI环境中检索
 * 
 *  注意：在即将到来的符合EE 7的WebLogic和WebSphere版本中,应该优先使用{@link orgspringframeworkschedulingconcurrentDefaultManagedTaskExecutor}
 * ,遵循Java EE 7中的JSR-236支持。
 * 
 * 
...
public class WorkManagerTaskExecutor extends JndiLocatorSupport
	 ...
	 * <p>
	 * 指定要委派给<p>的CommonJ WorkManager。或者,也可以指定目标WorkManager的JNDI名称
	 * 
	 * 
	 ...
	 * <p>
	 *  设置CommonJ WorkManager的JNDI名称<p>如果"resourceRef"设置为"true",则可以是完全限定的JNDI名称或相对于当前环境命名上下文的JNDI名称
	 * 
	 * 
	 * @see #setWorkManager
	 * @see #setResourceRef
	 ...
	 * <p>
	 *  指定要应用的CommonJ WorkListener(如果有)<p>此共享的WorkListener实例将通过此TaskExecutor上的所有{@link #execute}调用传递到WorkMan
	 * ager。
	 * 
	 */
