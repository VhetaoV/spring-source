 ...
 *
 * <p>
 *  希望将{@link orgaopallianceinterceptMethodInterceptor拦截器}添加到生成的bean的{@link orgspringframeworkbeansfactoryxmlBeanDefinitionDecorator BeanDefinitionDecorators}
 * 的基本实现。
 * 
 * <p>此基类控制{@link ProxyFactoryBean} bean定义的创建,并将原始包装作为{@link ProxyFactoryBean}的{@code target}属性的内部bean定义
 * 。
 * 
 *  <p>正确处理链接,确保仅创建一个{@link ProxyFactoryBean}定义如果先前的{@link orgspringframeworkbeansfactoryxmlBeanDefinitionDecorator}
 * 
 * @author Rob Harrop
...
public abstract class AbstractInterceptorDrivenBeanDefinitionDecorator implements BeanDefinitionDecorator {
	 ...
	 * <p>
	 * 已经创建了{@link orgspringframeworkaopframeworkProxyFactoryBean},则拦截器将被简单地添加到现有定义中。
	 * 
	 *  <p>子类只能为要添加的拦截器创建{@code BeanDefinition}
	 * 
	 */
