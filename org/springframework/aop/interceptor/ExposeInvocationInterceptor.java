 ...
 *
 * <p>
 * 拦截器将当前的{@link orgaopallianceinterceptMethodInvocation}暴露为线程本地对象我们偶尔需要这样做;例如,当切入点(例如AspectJ表达式切入点)需要知道
 * 完整的调用上下文时。
 * 
 *  <p>不要使用这个拦截器,除非这是真的必要的目标对象通常不应该知道Spring AOP,因为这创建了一个依赖于Spring API的目标对象应尽可能简单的POJO
 * 
 *  <p>如果使用,拦截器通常是拦截器链中的第一个拦截器
 * 
 * 
 * @author Rod Johnson
...
public class ExposeInvocationInterceptor implements MethodInterceptor, PriorityOrdered, Serializable {
	 ...
	 * <p>
	 *  此类的Singleton顾问在使用Spring AOP时优先使用INSTANCE,因为它阻止了创建一个新的Advisor来包装实例
	 * 
	 */
	 ...
	 * <p>
	 * 返回与当前调用关联的AOP联盟MethodInvocation对象
	 * 
	 * 
	 ...
	 * <p>
	 *  需要支持序列化替代使用规范实例进行反序列化,保护Singleton模式<p>替代{@code equals}方法
	 */
