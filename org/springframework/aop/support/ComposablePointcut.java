 ...
 *
 * <p>
 *  构建切入点的方便类所有方法都返回ComposablePointcut,因此我们可以使用简洁的习语,如：
 * 
 * {@code Pointcut pc = new ComposablePointcut()union(classFilter)intersection(methodMatcher)intersection(pointcut); }
 * 。
 * 
 * 
 * @author Rod Johnson
...
public class ComposablePointcut implements Pointcut, Serializable {
	 ...
	 * <p>
	 *  创建一个默认的ComposablePointcut,使用{@code ClassFilterTRUE}和{@code MethodMatcherTRUE}
	 * 
	 */
	 ...
	 * <p>
	 *  为给定的ClassFilter创建一个ComposablePointcut,使用{@code MethodMatcherTRUE}
	 * 
	 * 
	 ...
	 * <p>
	 *  为给定的MethodMatcher创建一个ComposablePointcut,使用{@code ClassFilterTRUE}
	 * 
	 * 
	 ...
	 * <p>
	 *  为给定的ClassFilter和MethodMatcher创建ComposablePointcut
	 * 
	 * 
	 * @param classFilter the ClassFilter to use
	 * @param methodMatcher the MethodMatcher to use
	 ...
	 * <p>
	 * 应用给定的Pointcut的联合<p>请注意,对于一个Pointcut联合,只有当原始ClassFilter(来自始发的Pointcut))匹配时,方法才会匹配,并且来自不同切入点的MethodMatc
	 * hers和ClassFilter将不会互相交错。
	 * 
	 * 
