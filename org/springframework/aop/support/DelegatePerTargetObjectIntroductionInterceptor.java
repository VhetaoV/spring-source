 ...
 * 
 * <p>与{@link DelegatingIntroductionInterceptor}不同之处在于,该类的单个实例可用于建议多个目标对象,并且每个目标对象将具有其自己的</i>委托(而Delegat
 * ingIntroductionInterceptor共享相同代表,因此所有目标的状态相同)。
 * 
 *  <p> {@code suppressInterface}方法可以用于抑制由委托类实现的接口,但不应该将其引入到拥有的AOP代理
 * 
 *  <p>如果代理是这个类的一个实例是可序列化的
 * 
 *  注意：此类与{@link DelegatingIntroductionInterceptor}之间有一些实现相似之处,建议可能重构以后提取一个共同的祖先类。</i>
 * 
 * 
