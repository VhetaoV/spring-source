 ...
 *
 * <p>
 * TargetSourceCreator为每个定义为"lazy-init"的bean强制执行LazyInitTargetSource这将导致为每个bean创建一个代理,允许在不实际初始化目标bean实例的
 * 情况下获取对这样的bean的引用。
 * 
 *  为自动代理创建者注册为自定义的TargetSourceCreator,与特定bean的自定义拦截器相结合,或仅用于创建延迟初始化代理。
 * 例如,作为XML应用程序上下文定义中的自动检测的基础架构bean：。
 * 
 * <pre class="code">
 * 
