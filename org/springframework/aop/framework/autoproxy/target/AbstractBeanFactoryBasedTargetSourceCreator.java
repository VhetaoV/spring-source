 ...
 *
 * <p>
 *  需要创建多个原型bean实例的{@link orgspringframeworkaopframeworkautoproxyTargetSourceCreator}实现方便的超类
 * 
 * <p>使用内部BeanFactory来管理目标实例,将原始bean定义复制到内部工厂这是必要的,因为原始BeanFactory将只包含通过自动代理创建的代理实例
 * 
 *  <p>需要在{@link orgspringframeworkbeansfactorysupportAbstractBeanFactory}中运行
 * 
 * 
 * @author Rod Johnson
...
public abstract class AbstractBeanFactoryBasedTargetSourceCreator
	 ...
	 * <p>
	 *  在Tar​​getSourceCreator关闭时销毁内部BeanFactory
	 * 
	 * 
	 ...
	 * <p>Default is "true".
	 * <p>
	 *  返回此TargetSourceCreator是否为基于原型的目标bean定义的范围将相应设置<p>默认值为"true"
	 * 
	 * 
	 ...
	 * <p>
	 * 子类必须实现这个方法来返回一个新的AbstractPrototypeBasedTargetSource,如果他们希望为这个bean创建一个自定义的TargetSource,或者{@code null}如
	 * 果他们不感兴趣,在这种情况下不会创建特殊的目标源子类不应该调用AbstractPrototypeBasedTargetSource上的{@code setTargetBeanName}或{@code setBeanFactory}
	 * 
