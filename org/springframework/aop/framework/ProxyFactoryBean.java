 ...
 *
 * <p>
 *  {@link orgspringframeworkbeansfactoryFactoryBean}实现,在Spring中构建基于bean的AOP代理{@link orgspringframeworkbeansfactoryBeanFactory}
 * 。
 * 
 * 通过"interceptorNames"属性指定的当前bean工厂中的bean名称列表来标识{@ link orgaopallianceinterceptMethodInterceptor MethodInterceptors}
 * 和{@link orgspringframeworkaopAdvisor Advisors}列表中的最后一个条目可以是目标的名称bean或{@link orgspringframeworkaopTargetSource}
 * ;但是,通常优选使用"targetName"/"target"/"targetSource"属性。
 * 
 * <p>可以在工厂级别添加全局拦截器和顾问指定的扩展名在拦截器列表中,列表中包含"xxx *"条目,将给定的前缀与bean名称匹配(例如"global *"将匹配"globalBean1"和"global
 * Bean2","*"所有定义的拦截器)匹配的拦截器根据其返回的顺序值应用,如果它们实现{@link orgspringframeworkcoreOrdered}接口。
 * 
 *  <p>在给定代理接口时创建JDK代理,如果没有给定实际目标类的CGLIB代理,请注意,只有当目标类没有最终方法时,后者才能生效,因为动态子类将在运行时创建
 * 
 * <p>可以将从该工厂获取的代理转换为{@link Advised},或获取ProxyFactoryBean引用并以编程方式操作它对于独立的现有原型引用将不起作用但是,它将适用于原型随后从工厂获得更改截取
 * 将立即在单例上工作(包括现有引用)但是,为了更改接口或目标,有必要从工厂获取新的实例。
 * 这意味着从工厂获取的单例实例不具有相同的对象身份但是,他们确实有相同的拦截器和目标,并且更改任何引用将会更改所有对象。
 * 
 * 
 * @author Rod Johnson
...
public class ProxyFactoryBean extends ProxyCreatorSupport
	 ...
	 * <p>
	 * 设置我们代理的接口的名称如果没有给出接口,将创建实际类的CGLIB <p>这实际上等同于"setInterfaces"方法,但镜像TransactionProxyFactoryBean的"setProx
	 * yInterfaces"。
	 * 
	 * 
	 * @see #setInterfaces
	 ...
	 * <p>
	 * 设置Advice / Advisor bean名称列表必须始终设置为在bean工厂中使用此工厂bean <p>引用的bean应为Interceptor,Advisor或Advice类型列表中的最后一个条
	 * 目可以是任何在工厂中的bean如果它既不是一个忠告也不是一个顾问,新添加一个SingletonTargetSource来包装它如果设置了"target"或"targetSource"或"targetNa
	 * me"属性,那么这个目标bean就不能被使用,在这种情况下, InterceptorNames"数组必须只包含Advice / Advisor bean名称<p> <b>注意：在"interceptor
	 * Names"列表中将目标bean指定为最终名称已被弃用,将在未来的Spring版本中删除</b>使用{ @link #setTargetName"targetName"}属性。
	 * 
	 * 
	 * @see org.aopalliance.intercept.MethodInterceptor
	 * @see org.springframework.aop.Advisor
	 ...
	 * <p>
	 * 设置目标bean的名称这是在"interceptorNames"数组末尾指定目标名称的替代方法<p>您还可以通过"target"/"targetSource"直接指定目标对象或TargetSource对
	 * 象,财产。
	 * 
	 * 
	 ...
	 * <p>
	 *  设置是否自动检测代理接口(如果没有指定)<p>默认值为"true"关闭此标志,以便在没有指定接口时为完整目标类创建CGLIB代理
	 * 
	 * 
	 ...
	 * <p>
	 * 设置单例属性的值控制这个工厂是否应该总是返回相同的代理实例(这意味着相同的目标)或者是否应该返回一个新的原型实例,这意味着目标和拦截器也可能是新的实例,如果它们从原型bean定义获得这允许精细控制对象图
	 * 中的独立性/唯一性。
	 * 
	 */
	 ...
	 * <p>
	 * 返回代理当客户端从此工厂bean获取bean时调用创建要由此工厂返回的AOP代理的实例实例将被缓存为单例,并在每次调用{@code getObject()}时为代理创建
	 * 
	 * 
	 ...
	 * <p>
	 *  返回代理的类型将检查单例实例(如果已经创建),否则返回到代理接口(仅为一个),目标bean类型或TargetSource的目标类
	 * 
	 * 
	 ...
	 * <p>
	 *  返回此类的代理对象的单例实例,如果尚未创建,则懒洋洋地创建它
	 * 
	 * 
	 ...
	 * custom class loader.
	 * <p>
	 *  返回代理对象以公开<p>默认实现使用工厂的bean类加载器使用{@code getProxy}调用可以覆盖以指定自定义类加载器
	 * 
	 * 
	 ...
	 * <p>
	 *  检查interceptorNames列表是否包含目标名称作为最终元素如果找到,请从列表中删除最终名称并将其设置为targetName
	 * 
	 */
	 ...
	 * or may be a target.
	 * <p>
	 *  查看bean工厂元数据来确定这个bean名称是否是Advisor或Advice,或者可能是一个目标
	 * 
	 * 
	 ...
	 * <p>
	 * 创建顾问(拦截器)链来自BeanFactory的顾问将在每次添加新的原型实例时刷新。通过工厂API以编程方式添加的拦截器不受此类更改的影响
	 * 
	 */
	 ...
	 * <p>
	 *  返回一个独立的顾问链我们需要在每次返回新的原型实例时执行此操作,以返回原型顾问和建议的不同实例
	 * 
	 */
	 ...
	 * more strongly.
	 * <p>
	 *  在创建建议链时调用<p>将给定的建议,顾问或对象添加到拦截器列表由于这三种可能性,我们不能更强烈地键入签名
	 * 
	 * 
	 ...
	 * <p>
	 * 返回一个TargetSource在创建代理时使用如果目标没有在interceptorNames列表的末尾被指定,TargetSource将是这个类的TargetSource成员否则,如果需要,我们获取目
	 * 标bean并将其包装在TargetSource中。
	 * 
	 */
	 ...
	 * <p>
	 *  将来自调用getBean()的以下对象转换为intervisorNames数组中的名称到Advisor或TargetSource
	 * 
	 */
