 ...
 *
 * <p>
 * 用于池化{@link orgspringframeworkaopTargetSource}实现的抽象基类,用于维护目标实例池,从每个方法调用的池中获取和释放目标对象此抽象基类独立于具体的池化技术;有关具
 * 体示例,请参阅{@link CommonsPool2TargetSource}子类。
 * 
 *  <p>子类必须根据所选对象池实现{@link #getTarget}和{@link #releaseTarget}方法从{@link AbstractPrototypeBasedTargetSource}
 * 继承的{@link #newPrototypeInstance())方法可用于创建对象以便将它们放入游泳池。
 * 
 * <p>子类还必须从{@link PoolingConfig}接口中实现一些监控方法{@link #getPoolingConfigMixin()}方法使这些统计信息通过IntroductionAdvis
 * or可以在代理对象上使用。
 * 
 *  <p>此类实现{@link orgspringframeworkbeansfactoryDisposableBean}接口,以强制子类实现{@link #destroy()}方法,关闭其对象池
 * 
 * 
 * @author Rod Johnson
...
public abstract class AbstractPoolingTargetSource extends AbstractPrototypeBasedTargetSource
	 ...
	 * <p>
	 *  返回一个IntroductionAdvisor,它提供一个mixin来显示由该对象维护的池的统计信息
	 */
