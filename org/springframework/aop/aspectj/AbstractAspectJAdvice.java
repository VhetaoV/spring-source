 ...
 *
 * <p>
 *  包含AspectJ方面或AspectJ注释的建议方法的AOP联盟{@link orgaopallianceaopAdvice}类的基类
 * 
 * 
 * @author Rod Johnson
...
public abstract class AbstractAspectJAdvice implements Advice, AspectJPrecedenceInformation, Serializable {
	 ...
	 * <p>
	 *  Key在ReflectiveMethod中使用Invocation userAtributes当前连接点的映射
	 * 
	 */
	 ...
	 * (in an around advice).
	 * <p>
	 * Lazily实例化当前调用的连接点要求使用ExposeInvocationInterceptor绑定MethodInvocation <p>不要使用当前ReflectiveMethodInvocatio
	 * n(在周围的建议中)访问可用。
	 * 
	 * 
	 ...
	 * <p>
	 *  定义此建议的方面(ref bean)的名称(在确定优先级时使用,以便我们可以确定两条建议是否来自同一方面)
	 * 
	 */
	 ...
	 * <p>
	 *  如果此建议对象的创建者知道参数名称并明确设置它们,则该值将为非空值
	 * 
	 */
	 ...
	 * <p>
	 *  thisJoinPoint参数的索引(目前仅在索引0时支持,如果存在)
	 * 
	 */
	 ...
	 * supported at index 0 if present at all)
	 * <p>
	 *  thisJoinPointStaticPart参数的索引(当前仅在索引0时支持,如果存在)
	 * 
	 */
	 ...
	 * <p>
	 *  如果参数名称已知,则由该建议对象的创建者设置<p>这可能是因为它们已经以XML或者建议注释明确指定
	 * 
	 * 
	 ...
	 * <p>
	 *  我们需要在此级别保留返回名称以进行参数绑定计算,此方法允许afterReturning advice子类设置名称
	 * 
	 */
	 ...
	 * <p>
	 * 我们需要在这个级别保留投掷名称以进行参数绑定计算,该方法允许afterThrowing advice子类设置名称
	 * 
	 */
	 ...
	 * <p>
	 * 尽可能多的工作作为设置的一部分,以便后续的通知调用绑定的参数可以尽可能快<p>如果第一个参数是JoinPoint或ProceedingJoinPoint类型,那么我们在该位置传递一个JoinPoint(
	 *  ProceedingJoinPoint for around advice)<p>如果第一个参数的类型为{@code JoinPointStaticPart},那么我们在该位置传递{@code JoinPointStaticPart}
	 *  <p>剩余参数必须在给定连接点的切入点评估中被约束将从参数名称返回一个映射到值我们需要计算哪个参数需要绑定到哪个参数名称有多种策略来确定这个绑定,它们被排列在一个ChainOfRespability。
	 * 
	 */
	 ...
	 * <p>
	 * 创建一个ParameterNameDiscoverer用于参数绑定<p>默认实现创建一个{@link DefaultParameterNameDiscoverer}并添加一个专门配置的{@link AspectJAdviceParameterNameDiscoverer}
	 * 。
	 * 
	 */
	 ...
	 * <p>
	 *  来自argumentIndexOffset的所有参数向前是切入点参数的候选项,但返回和抛出的vars的处理方式不同,如果存在则必须从列表中删除
	 * 
	 */
