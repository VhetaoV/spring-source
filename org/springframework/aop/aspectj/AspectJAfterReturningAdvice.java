 ...
 *
 * <p>
 *  Spring AOP建议包装一个AspectJ后返回通知方法
 * 
 * 
 * @author Rod Johnson
...
public class AspectJAfterReturningAdvice extends AbstractAspectJAdvice
	 ...
	 * <p>
	 * 根据AspectJ语义,如果指定了返回的子句,则只有在返回的值是给定的返回类型的实例和通用类型参数(如果有)匹配赋值规则的情况下才会调用该通知。如果返回类型为Object,则建议是*总是被调用
	 * 
	 * 
	 ...
	 * <p>
	 *  遵循AspectJ语义,如果返回值为null(或返回类型为void),则应使用目标方法的返回类型来确定是否调用了通知。
	 * 另外,即使返回类型为void,如果类型为在advice方法中声明的参数是Object,那么建议仍然需要被调用。
	 * 
