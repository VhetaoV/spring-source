 ...
 *
 * <p>
 * 提供消息源实现的基类,提供诸如{@link javatextMessageFormat}之类的支持基础设施,但不能实现{@link orgspringframeworkcontextMessageSource}
 * 中定义的具体方法,。
 * 
 *  <p> {@ link AbstractMessageSource}源自这个类,提供具体的{@code getMessage}实现,委托给一个中央模板方法来消息代码解析
 * 
 * 
...
public abstract class MessageSourceSupport {
	 ...
	 * <p>
	 *  缓存保存已生成的MessageFormats每个消息用于传入的默认消息解析代码的MessageFormats在特定的基础上缓存在子类中
	 * 
	 */
	 ...
	 * <p>
	 * 设置是否始终应用MessageFormat规则,解析没有参数的偶数消息<p>默认值为"false"：默认情况下,没有参数的消息将按原样返回,而不通过MessageFormat进行解析将其设置为"true
	 * "以强制实施MessageFormat消息,期望使用MessageFormat转义所有消息文本<p>例如,MessageFormat期望单引号被转义为"""如果您的消息文本全部用这样的转义写入,即使没有
	 * 定义参数占位符,您也可以需要将此标志设置为"true"否则,只有具有实际参数的消息文本应该用MessageFormat转义。
	 * 
	 * 
	 ...
	 * messages without arguments.
	 * <p>
	 *  返回是否始终应用MessageFormat规则,解析没有参数的偶数消息
	 * 
	 */
	 ...
	 * <p>
	 * 渲染给定的默认消息字符串默认消息按照调用者的指定传入,并且可以呈现给用户显示的完全格式化的默认消息<p>默认实现将String传递给{@code formatMessage},解析任何参数在其中找到占位
	 * 符子类可以覆盖此方法来插入默认消息的自定义处理。
	 * 
	 * 
	 ...
	 * <p>
	 *  使用缓存的MessageFormats格式化给定的消息String默认情况下,对传入的默认消息进行调用,以解决在其中发现的任何参数占位符
	 * 
	 * 
	 ...
	 * <p>
	 * 用于解析参数对象的模板方法<p>默认实现只是返回给定的参数数组,因为可以在子类中被覆盖,以便解析特殊的参数类型
	 * 
