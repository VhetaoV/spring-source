 ...
 *
 * <p>
 * 适用于活动bean视图曝光,从本地{@code ApplicationContext}(具有本地{@code LiveBeansView} bean定义)或所有注册的ApplicationContext
 * s(由{@value #MBEAN_DOMAIN_PROPERTY_NAME驱动)构建当前bean及其依赖关系的快照}环境属性)。
 * 
 *  <p>注意：此功能仍处于测试阶段,主要用于与Spring Tool Suite 31及更高版本配合使用
 * 
 * 
...
public class LiveBeansView implements LiveBeansViewMBean, ApplicationContextAware {
	 ...
	 * <p>
	 *  生成当前bean及其依赖关系的JSON快照,通过{@link #findApplicationContexts()}查找所有活动的ApplicationContexts,然后委派给{@link #generateJson(javautilSet)}
	 * }。
	 * 
	 */
	@Override
	 ...
	 * <p>
	 *  查找当前应用程序的所有适用的ApplicationContexts <p>如果没有为此LiveBeansView设置特定的ApplicationContext,则调用
	 * 
	 * 
	 ...
	 * <p>
	 * 实际上在给定的ApplicationContexts中生成一个bean的JSON快照<p>此实现不使用任何JSON解析库,以避免第三方库依赖性它生成一个上下文描述对象数组,每个包含一个上下文和父属性以及
	 * 带有嵌套bean描述对象的beans属性每个bean对象都包含一个bean,scope,type和resource属性,以及一个依赖关系属性,它具有一个bean列表的嵌套数组,。
	 * 
	 * 
	 ...
	 * <p>
	 *  确定给定的b​​ean定义的资源描述,并将基本的JSON转义(反斜杠,双引号)应用到它
	 * 
