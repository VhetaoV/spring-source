 ...
 *  表示给定bean的"角色"提示
 * 
 *  <p>可用于直接或间接使用{@link orgspringframeworkstereotypeComponent}注释的任何类或使用{@link Bean}注释的方法
 * 
 * <p>如果组件或Bean定义中不存在此注释,则将应用默认值{@link BeanDefinition#ROLE_APPLICATION}
 * 
 * 
 * @author Chris Beams
 * @since 3.1
...
public @interface Role {
	 ...
	 * <p>
	 *  <p>如果{@link Configuration @Configuration}类中存在角色,则表示配置类bean定义的角色,不会级联到所有定义的@ {@ code Bean}方法。
	 * 此行为与@ {@ link Lazy}注释,例如。
	 * 
	 * 
	 * @see BeanDefinition#ROLE_APPLICATION
	 * @see BeanDefinition#ROLE_INFRASTRUCTURE
