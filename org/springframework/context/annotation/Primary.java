 ...
 *
 * <p>
 * 表示当多个候选人有资格自动连接单值依赖关系时,应该给予一个优先级如果候选者之间只存在一个"主"bean,则它将是自动连线值该注释在语义上等同于{@code <bean >}元素的{@code primary}
 * 属性。
 * 
 *  <p>可以用于直接或间接用@ {@ link orgspringframeworkstereotypeComponent Component}注释的任何类或者使用@ {@ link Bean}注释的方
 * 法。
 * 
 *  <H2>实施例</H2>
 * <pre class="code">
 *  @Component public class FooService {private FooRepository fooRepository;
 * 
 *  @Autowired public FooService(FooRepository fooRepository){thisfooRepository = fooRepository; }}
 * 
 * 
 * @author Chris Beams
 * @since 3.0
