 ...
 *
 * <p>
 *  基本的{@link ConfigurablePropertyAccessor}为所有典型用例提供必要的基础设施
 * 
 * <p>如果需要,此访问器将将集合和数组值转换为相应的目标集合或数组。
 * 处理集合或数组的自定义属性编辑器可以通过PropertyEditor的{@code setValue}或通过逗号分隔的String通过{@code setAsText},因为如果数组本身不可分配,Str
 * ing数组将以这种格式转换。
 * <p>如果需要,此访问器将将集合和数组值转换为相应的目标集合或数组。
 * 
 * 
...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	 ...
	 * <p>
	 *  我们将创建很多这些对象,所以我们不需要每次都有一个新的记录器
	 * 
	 */
	 ...
	 * <p>
	 *  指定数组和集合自动增长的限制<p>默认值在普通访问器上是无限制的
	 * 
	 */
	 ...
	 * <p>
	 *  切换目标对象,只有当新对象的类与被替换对象的类不同时,才能替换缓存的内省结果
	 * 
	 * 
	 ...
	 * <p>
	 *  返回指定的{@code propertyName}的{@link PropertyHandler},如有必要,导航返回{@code null}(如果没有找到)而不是抛出异常
	 * 
	 * 
	 ...
	 * <p>
	 *  为指定的本地{@code propertyName}返回{@link PropertyHandler}仅用于达到当前上下文中可用的属性
	 * 
	 * 
	 ...
	 * <p>
	 *  创建新的嵌套属性访问器实例可以在子类中覆盖以创建PropertyAccessor子类
	 * 
	 * 
	 ...
	 * <p>
	 * 检索给定嵌套属性的属性访问器如果没有找到缓存,请创建一个新的<p>注意：现在需要缓存嵌套的PropertyAccessors,以便为嵌套属性保留注册的自定义编辑器
	 * 
	 * 
