 ...
 *
 * <p>
 *  {@link PropertyEditorRegistry}接口的基本实现提供对默认编辑器和自定义编辑器的管理主要用作{@link BeanWrapperImpl}的基类
 * 
 * 
...
public class PropertyEditorRegistrySupport implements PropertyEditorRegistry {
	 ...
	 * <p>
	 *  激活仅用于配置目的的配置值编辑器,例如{@link orgspringframeworkbeanspropertyeditorsStringArrayPropertyEditor} <p>这些编辑器默
	 * 认情况下不会注册,因为它们通常不适合数据绑定目的。
	 * 当然,您可以单独注册任何通过{@link #registerCustomEditor}。
	 * 
	 */
	 ...
	 * <p>
	 * 使用给定的属性编辑器覆盖指定类型的默认编辑器<p>请注意,这与注册自定义编辑器不同,因为编辑器语义上仍然是默认编辑器。
	 ...
	 * <p>
	 *  检索给定属性类型的默认编辑器,如果任何<p> Lazily注册默认编辑器,如果它们处于活动状态
	 * 
	 * 
	 ...
	 * <p>
	 * 确定给定属性路径的属性类型<p>如果未指定必需的类型,则由{@link #findCustomEditor}调用,即使只给出属性路径,也可以找到类型特定的编辑器<p>默认值实现总是返回{@code null}
	 *  BeanWrapperImpl将使用BeanWrapper接口定义的标准{@code getPropertyType}方法来覆盖此方法。
	 * 
	 * 
	 ...
	 * <p>
	 *  获取给定类型的自定义编辑器如果没有找到直接匹配,请尝试使用超类的自定义编辑器(在任何情况下都可以通过{@code getAsText}将值渲染为String)
	 * 
	 * 
	 * @param requiredType the type to look for
	 ...
	 * <p>
	 *  从注册的自定义编辑器猜测指定属性的属性类型(前提是它们已注册为特定类型)
	 * 
	 * 
	 ...
	 * <p>
	 *  添加带有剥离键和/或索引的所有变体的属性路径使用嵌套路径递归自身调用
	 * 
	 * 
	 ...
	 * <p>
	 *  拥有属性名称的注册自定义编辑器的持有人保留PropertyEditor本身以及注册的类型
	 */
