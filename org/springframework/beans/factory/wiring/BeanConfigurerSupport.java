 ...
 * <p>
 *  可以对对象执行依赖注入的方便的基类(但是可以创建它们)通常由AspectJ方面子类化
 * 
 * <p>子类在{@link BeanWiringInfoResolver}界面中也可能需要自定义元数据解析策略默认实现会查找与完全限定类名称相同名称的bean(这是一个bean的默认名称如果未使用"{@code id}
 * "属性,则为Spring XML文件)。
 * 
 * 
 * @author Rob Harrop
...
public class BeanConfigurerSupport implements BeanFactoryAware, InitializingBean, DisposableBean {
	 ...
	 * <p>
	 *  将{@link BeanWiringInfoResolver}设置为使用<p>默认行为是查找与该类名称相同的bean。另外,请考虑使用注释驱动的bean接线
	 * 
	 * 
	 * @see ClassNameBeanWiringInfoResolver
	 ...
	 * <p>
	 *  设置此方面必须配置bean的{@link BeanFactory}
	 * 
	 */
	@Override
	 ...
	 * <p>
	 *  创建默认BeanWiringInfoResolver以使用,如果没有明确指定<p>默认实现构建一个{@link ClassNameBeanWiringInfoResolver}
	 * 
	 * 
	 ...
	 * <p>
	 * 当容器被销毁时,发布对{@link BeanFactory}和{@link BeanWiringInfoResolver}的引用
	 * 
	 */
	@Override
	public void destroy() {
	 ...
	 * <p>
	 *  配置bean实例<p>子类可以覆盖此值以提供自定义配置逻辑通常由一个方面调用,对于所有由切入点匹配的bean实例
	 * 
