 ...
 *
 * <p>
 *  用于Spring bean定义的基于Groovy的读取器：像Groovy构建器,但是更多的是用于Spring配置的DSL
 * 
 * <p>此bean定义阅读器还可以了解XML bean定义文件,允许与Groovy bean定义文件进行无缝混合和匹配
 * 
 ...
 * 
 * def reader = new GroovyBeanDefinitionReader(myApplicationContext)readerbeans {dataSource(BasicDataSource){// <--- invokeMethod driverClassName ="orghsqldbjdbcDriver"url ="jdbc：hsqldb：mem：grailsDB"username ="sa"// < -  setProperty password =""settings = [mynew："setting"]} sessionFactory(SessionFactory){dataSource = dataSource // < - 用于检索引用的getProperty)myService(MyService){nestedBean = {AnotherBean bean  - > // < -  setProperty with封闭为嵌套的bean dataSource = dataSource}}} </pre>
 * 。
 * 
 * 您还可以使用{@link #loadBeanDefinitions(Resource)}或{@link #loadBeanDefinitions(String)}方法加载包含在Groovy脚本中定义的B
 * ean的资源,脚本看起来类似于以下内容。
 * 
 * <pre class="code">
 *  import orghibernateSessionFactory import orgapachecommonsdbcpBasicDataSource
...
public class GroovyBeanDefinitionReader extends AbstractBeanDefinitionReader implements GroovyObject {
	 ...
	 * <p>
	 *  使用从XML文件加载bean定义的默认设置创建的标准{@code XmlBeanDefinitionReader}
	 * 
	 */
	 ...
	 * <p>
	 *  根据给定的{@link XmlBeanDefinitionReader}创建一个新的{@code GroovyBeanDefinitionReader},将bean定义加载到其{@code BeanDefinitionRegistry}
	 * 中,并将Groovy DSL加载委托给它<p>提供的{@code XmlBeanDefinitionReader}配置了禁用XML验证。
	 * 
	 * 
	 ...
	 * <p>
	 *  设置绑定,即Groovy变量在{@code GroovyBeanDefinitionReader}关闭的范围内可用
	 * 
	 */
	 ...
	 * <p>
	 * 从指定的Groovy脚本或XML文件加载bean定义<p>请注意,{@code"xml"}文件将被解析为XML内容;所有其他类型的资源将被解析为Groovy脚本
	 * 
	 * 
	 ...
	 * <p>
	 *  将Spring bean定义从XML或Groovy源导入到当前bean构建器实例中
	 * 
	 * 
	 ...
	 * <p>
	 *  该方法覆盖方法调用,以创建使用类参数的每个方法名称的bean
	 * 
	 */
	 ...
	 * <p>
	 *  检查{@link Map}中是否有{@link RuntimeBeanReference},如有必要,将其转换为{@link ManagedMap}
	 * 
	 * 
	 ...
	 * <p>
	 *  检查{@link列表}中是否存在{@link RuntimeBeanReference},如果需要,将其转换为{@link ManagedList}
	 * 
	 * 
	 ...
	 * </ul>
	 * <p>
	 *  此方法将{@code GroovyBeanDefinitionReader}的范围中的属性检索覆盖为：
	 * <ul>
	 * <li>如果存在,则从bean构建器的绑定中检索变量<li>检索特定bean的RuntimeBeanReference(如果存在)<li>否则,只需委托MetaClassgetProperty即可从{@code GroovyBeanDefinitionReader}
	 * 本身解析属性。
	 * </ul>
	 */
	 ...
	 * <p>
	 *  此类用于延迟将属性添加到bean定义,直到此为止。此属性用于将属性分配给列表,该列表可能不包含该分配点的bean引用,但可能稍后使用;因此,需要进行管理
	 * 
	 */
	...
	private class GroovyRuntimeBeanReference extends RuntimeBeanReference implements GroovyObject {
		 ...
		 * <p>
		 *  包装一个bean定义属性一个确保任何RuntimeBeanReference添加到它后面被推迟解析
		 */
