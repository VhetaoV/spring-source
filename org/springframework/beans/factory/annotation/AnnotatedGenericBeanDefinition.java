 ...
 *
 * <p>
 *  扩展{@link orgspringframeworkbeansfactorysupportGenericBeanDefinition}类,添加对通过{@link AnnotatedBeanDefinition}
 * 界面公开的注释元数据的支持。
 * 
 * 此GenericBeanDefinition变体主要用于测试期望在AnnotatedBeanDefinition上运行的代码,例如Spring组件扫描支持中的策略实现(默认定义类为{@link orgspringframeworkcontextannotationScannedGenericBeanDefinition}
 * ),还实现了AnnotatedBeanDefinition接口)。
 * 
 * 
...
public class AnnotatedGenericBeanDefinition extends GenericBeanDefinition implements AnnotatedBeanDefinition {
	 ...
	 * <p>
	 * 为给定的注释元数据创建一个新的AnnotatedGenericBeanDefinition,允许基于ASM的处理和避免早期加载bean类注意,该构造函数在功能上等同于{@link orgspringframeworkcontextannotationScannedGenericBeanDefinition ScannedGenericBeanDefinition}
	 * ,但后者的语义表示一个bean是通过组件扫描专门发现的,而不是其他方式。
	 * 
	 * 
