 ...
 *
 * <p>
 *  界面由{@link DefaultBeanDefinitionDocumentReader}用于处理自定义嵌套(直接在{@code <bean>})标签下
 * 
 * <p>装饰也可能基于应用于{@code <bean>}标签的自定义属性进行。
 * 实现可以根据需要将自定义标签中的元数据自由地转换为尽可能多的{@link orgspringframeworkbeansfactoryconfigBeanDefinition BeanDefinitions}
 * ,并将{@链接orgspringframeworkbeansfactoryconfigBeanDefinition},可能甚至返回完全不同的{@link orgspringframeworkbeansfactoryconfigBeanDefinition}
 * 来替换原始的。
 * <p>装饰也可能基于应用于{@code <bean>}标签的自定义属性进行。
 * 
 * </p> {@ link BeanDefinitionDecorator BeanDefinitionDecorators}应该意识到它们可能是链的一部分特别是,{@link BeanDefinitionDecorator}
 * 应该意识到以前的{@link BeanDefinitionDecorator}可能已经替换了原来的{@link orgspringframeworkbeansfactoryconfigBeanDefinition}
 * 使用{@link orgspringframeworkaopframeworkProxyFactoryBean}定义,允许添加自定义{@link orgaopallianceinterceptMethodInterceptor拦截器}
 * 。
 * 
 * @author Rob Harrop
 * @since 2.0
...
public interface BeanDefinitionDecorator {
	 ...
	 * <p>
	 * 
	 * 希望将拦截器添加到封闭bean的<p> {@ link BeanDefinitionDecorators}应该扩展处理链接的{@link orgspringframeworkaopconfigAbstractInterceptorDrivenBeanDefinitionDecorator}
	 * ,确保只创建一个代理,并且包含链中的所有拦截器。
	 * 
	 *  <p>解析器从{@link NamespaceHandler}中为自定义标签所在的命名空间找到一个{@link BeanDefinitionDecorator}
	 * 
	 */
