 ...
 *
 * <p>
 *  适配器实现{@link DisposableBean}和{@link Runnable}接口在给定的bean实例上执行各种销毁步骤：
 * <ul>
 * <LI> DestructionAwareBeanPostProcessors; <li>实现DisposableBean本身的bean; <li>在bean定义中指定的自定义销毁方法
 * </ul>
 * 
 * 
...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
	 ...
	 * <p>
	 * 如果给定的beanDefinition的"destroyMethodName"属性的当前值是{@link AbstractBeanDefinition#INFER_METHOD},那么尝试推断出一个破坏
	 * 方法Candidate方法目前仅限于public,no-arg方法,名为"close"或"shutdown"在本地或继承的情况下被声明)给定的BeanDefinition的"destroyMethodN
	 * ame"如果没有找到这样的方法,则更新为null,否则设置为推断方法的名称该常量作为{@code @ Bean#destroyMethod}属性的默认值,常量的值也可以在{@code <bean destroy-method =""}或{@code <beans default-destroy-method ="">}
	 * 属性中的XML中使用)还处理{ @link javaioCloseable}和{@link javalangAutoCloseable}接口,反映了实现bean的"close"方法。
	 * 
	 */
	 ...
	 * <p>
	 * 搜索列表中的所有DestructionAwareBeanPostProcessors
	 * 
	 * 
	 ...
	 * <p>
	 *  在给定的bean上调用指定的自定义destroy方法<p>如果找到,此实现将调用无参数方法,否则检查具有单个布尔参数的方法(假设"force"参数传入"true"),否则记录错误
	 * 
	 */
	 ...
	 * <p>
	 *  序列化此类状态的副本,过滤出不可序列化的BeanPostProcessors
	 * 
	 */
