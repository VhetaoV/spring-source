 ...
 *
 * <p>
 * 运行时{@link BeanPostProcessor}实现的<i>合并</i> bean定义的后处理器回调接口可以实现此子接口,以便后处理Spring BeanFactory用于创建特定bean的合并
 * bean定义例。
 * 
 *  <p> {@link #postProcessMergedBeanDefinition}方法可以例如内省bean定义,以便在后处理bean的实际实例之前准备一些缓存的元数据。
 * 还允许修改bean定义,但仅限<i> </i>用于实际用于并发修改的bean定义属性基本上,这仅适用于在{@link RootBeanDefinition}本身定义的操作,而不适用于其基类的属性。
 * 
