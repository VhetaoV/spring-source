 ...
 *
 * <p>
 *  用于解析构造函数和工厂方法的代理通过参数匹配执行构造函数解析
 * 
 * 
...
class ConstructorResolver {
	 ...
	 * <p>
	 * "autowire构造函数"(带有构造函数参数的类型)行为也适用于显式构造函数参数值,与bean工厂匹配所有剩余参数与bean对应于构造函数注入：在这种模式下,Spring bean工厂是能够托管希望基
	 * 于构造函数的依赖关系解析的组件。
	 * 
	 * 
	 ...
	 * <p>
	 *  在指定的bean定义中解决工厂方法,如果可能,可以检查结果为{@link RootBeanDefinition#getResolvedFactoryMethod()})
	 * 
	 * 
	 ...
	 * <p>
	 *  检索给定类的所有候选方法,考虑到{@link RootBeanDefinition#isNonPublicAccessAllowed()}标志被称为工厂方法确定的起点
	 * 
	 */
	 ...
	 * <p>
	 * 使用命名工厂方法实例化bean如果bean定义参数指定一个类而不是"factory-bean",或者使用依赖注入配置的工厂对象上的实例变量,则该方法可能是静态的使用RootBeanDefinition中
	 * 指定的名称(该方法可能被重载)迭代静态或实例方法,并尝试与参数匹配我们没有附加到构造函数args的类型,因此尝试和错误是唯一的方法here这个explicitArgs数组可能包含通过相应的getBean
	 * 方法以编程方式传递的参数值。
	 * 
	 * 
	 * @param beanName the name of the bean
	 ...
	 * <p>
	 * 将此bean的构造函数参数解析为resolvedValues对象这可能涉及查找其他bean <p>此方法也用于处理静态工厂方法的调用
	 * 
	 */
			...
			if (valueHolder != null) {
				...
				if (valueHolder.isConverted()) {
					...
					try {
						...
						/* <p>
						/*  if(originalValue == sourceValue || sourceValue instanceof TypedStringValue){//转换后的值还是原来的值：存储转换值sourceHoldersetConvertedValue(convertedValue); argspreparedArguments [paramIndex] = convertedValue; }
						/*  else {。
						/* 
						*/
