...
public class CustomMapEditor extends PropertyEditorSupport {
	 ...
	 * <p>
	 *  为给定的目标类型创建一个新的CustomMapEditor,按原样保留传入的{@code null}
	 * 
	 * 
	 ...
	 * <p>
	 * 为给定的目标类型创建一个新的CustomMapEditor <p>如果传入值为给定类型,则将按原样使用。
	 * 如果它是不同的Map类型或数组,则将转换为默认实现给定地图类型如果该值为其他值,则将创建具有该单个值的目标Map <p>默认Map实现为：SortedMap的TreeMap和Map的LinkedHash
	 * Map。
	 * 为给定的目标类型创建一个新的CustomMapEditor <p>如果传入值为给定类型,则将按原样使用。
	 * 
	 * 
	 ...
	 * <p>
	 * 返回是否始终创建新的Map,即使传入的Map的类型已经匹配<p> Default是"false";可以覆盖以强制创建新的Map,例如在任何情况下转换元素
	 * 
	 * 
	 * @see #convertKey
	 * @see #convertValue
	 ...
	 * <p>
	 *  钩子转换每个遇到的Map键默认实现只是按原样返回传入键<p>可以覆盖某些键的转换,例如从String到Integer <p>只有在实际创建新的Map时才调用！默认情况下,如果传入地图的类型已经匹配Ov
	 * erride {@link #alwaysCreateNewMap()},以强制在每种情况下创建一个新的Map。
	 * 
	 * 
	 ...
	 * enforce creating a new Map in every case.
	 * <p>
	 * 钩子转换每个遇到的Map值默认实现只是按原样返回传入值<p>可以覆盖以执行某些值的转换,例如从String到Integer <p>仅在实际创建新的Map时调用！默认情况下,如果传入地图的类型已经匹配Ov
	 * erride {@link #alwaysCreateNewMap()},以强制在每种情况下创建一个新的Map。
	 * 
	 * 
