 ...
 *
 * <p>
 *  {@link PropertyValues}界面的默认实现允许对属性进行简单的操作,并提供构造函数来支持从Map​​的深层复制和构造
 * 
 * 
 * @author Rod Johnson
...
public class MutablePropertyValues implements PropertyValues, Serializable {
	 ...
	 * <p>
	 * 创建一个新的空的MutablePropertyValues对象<p>可以使用{@code add}方法添加属性值
	 * 
	 * 
	 ...
	 * <p>
	 *  深层拷贝构造函数保证PropertyValue引用是独立的,尽管它不能深入复制由PropertyValue对象当前引用的对象
	 * 
	 * 
	 ...
	 * <p>
	 *  使用给定的PropertyValue对象列表构造一个新的MutablePropertyValues对象as-is <p>这是高级使用场景的构造函数它不适用于典型的编程使用
	 * 
	 * 
	 ...
	 * It is not intended for typical programmatic use.
	 * <p>
	 * 以原始形式返回PropertyValue对象的底层列表返回的列表可以直接修改,但不建议使用<p>这是对所有PropertyValue对象进行优化访问的访问器,它不适用于典型的程序化使用
	 * 
	 */
	 ...
	 * referenced by individual PropertyValue objects.
	 * <p>
	 *  将所有给定的PropertyValues复制到此对象中保证PropertyValue引用是独立的,尽管它不能深入复制当前由PropertyValue对象引用的对象
	 * 
	 * 
	 ...
	 * <p>
	 * 注意：从Spring 30开始,我们建议使用更简洁,更可链接的变体{@link #add},该属性名称和属性值为{@code addPropertyValue}
	 * 
	 * 
	 ...
	 * Indexed from 0.
	 * <p>
	 *  修改此对象中保存的PropertyValue对象,从0变为Indexed
	 * 
	 */
	 ...
	 * <p>
	 *  拥有属性名称的{@code removePropertyValue}的重载版本
	 * 
	 * 
	 * @param propertyName name of the property
	 ...
	 * <p>
	 * 将指定的属性注册为"处理",在某些处理器的意义上,调用PropertyValue(s)机制之外的相应setter方法<p>这将导致从{@link #contains}调用返回{@code true}对于
	 * 指定的属性。
	 * 
	 * 
	 ...
	 * <p>
	 *  返回此持有人是否仅包含转换的值({@code true}),还是需要转换值({@code false})
	 */
