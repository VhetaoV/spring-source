 ...
 * 
 * <p>按照<a href=\"http://javasuncom/docs/books/jls/third_edition/html/conversionshtml\"> Java语言规范</a>第56
 * 2节的定义处理转化和升迁,成瘾{@code BigDecimal} / {@ code BigInteger}管理：。
 * 
 * <p>如果任何操作数是引用类型,则执行拆箱转换(第518节)然后：<br>如果任一操作数的类型为{@code BigDecimal},另一个转换为{@code BigDecimal} < br>如果任一
 * 操作数的类型为double,另一个转换为double <br>否则,如果任一操作数的类型为float,另一个转换为float <br>如果任一操作数的类型为{@code BigInteger},另一个转
 * 
 * @author Andy Clement
...
public class OpMultiply extends Operator {
	 ...
	 * </ul>
	 * <p>
	 * 换为{@code BigInteger} <br>否则,如果任一操作数的类型为long,另一个转换为long <br>否则,两个操作数都将转换为int类型。
	 * 
	 */
	@Override
