 ...
 *
 * <p>
 *  在解析的Spring Expression Language格式表达式中的所有AST节点的公共超类型
 * 
 * 
 * @author Andy Clement
 * @since 3.0
...
public abstract class SpelNodeImpl implements SpelNode, Opcodes {
	 ...
	 * <p>
	 * 表示此表达式结果结果的类型描述符一旦知道它就立即设置对于文字节点,它立即被知道对于属性访问或方法调用,在该节点的一次评估之后是已知的描述符是像字节码形式,但稍微更容易使用它不包括后缀分号(非数组引用类型
	 ...
	 * <p>
	 *  检查节点是否可以被编译为字节码每个节点中的推理可能不同,但通常将涉及检查节点的退出类型描述符是否已知,并且任何相关的子节点是可编译的
	 * 
	 * 
	 ...
	 * on the stack.
	 * <p>
	 * 为提供的访问者生成此节点的字节码关于正在编译的当前表达式的上下文信息在代码流对象中可用例如,它将包括有关当前在堆栈上的对象类型的信息
	 * 
	 * 
	 ...
	 * <p>
	 *  生成处理构造指定方法的参数值的代码此方法将考虑调用的方法是否是一个varargs方法,如果那么参数值将被适当地打包成一个数组
	 * 
	 * 
	 ...
	 * <p>
	 *  请求一个参数来生成其字节码,然后用任何拳击/拆箱/复选框来跟踪它,以确保它符合预期的参数描述符
	 */
