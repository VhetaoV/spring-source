 ...
 *
 * <p>
 * 表示对值的引用使用引用可以获取或设置值传递值引用而不是值本身可以避免操作数评估的不正确的重复例如在'list [index ++] ++'中没有值引用'list [index ++]'有必要对列表[in
 * dex ++]进行两次评估(一次获取该值,一次确定值在哪里),并将双倍递增索引。
 * 
 * 
 * @author Andy Clement
 * @since 3.2
...
public interface ValueRef {
	 ...
	 * re-evaluation.
	 * <p>
	 *  设置ValueRef指向的值,它不应该需要表达式组件重新评估
	 * 
	 * 
