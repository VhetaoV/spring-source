...
public class PropertyOrFieldReference extends SpelNodeImpl {
	 ...
	 * <p>
	 * 确定应用于尝试访问指定目标类型上的属性的属性解析器集合解析器被认为是有序列表,但是在返回的列表中,与输入目标类型完全匹配的任何一个到可以适用于任何类型的"一般"解析器)被放置在列表的开头。
	 * 另外,有特定的解析器,正确地命名了有问题的类和解析器,命名一个特定的类,但它是类的超类型有这些是放在特定解析器集合的末尾,将在完全匹配的访问器之后但在通用访问器之前尝试。
	 * 
