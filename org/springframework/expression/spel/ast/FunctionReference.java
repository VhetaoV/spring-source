 ...
 *
 * <p>
 * 函数引用的形式是"#someFunction(a,b,c)"函数可以在正在评估的表达式之前的上下文中定义,或者使用lambda函数定义在表达式本身中定义例如：表达式中的Lambda函数定义："(#max
 *  = {| x,y | $ x> $ y?$ x：$ y}; max(2,3))"调用上下文定义的函数："#isEven(37)"函数也可以是静态的java方法,在调用表达式之前在上下文中注册。
 * 
 *  函数非常简单,参数不是定义的一部分(现在),所以名称必须是唯一的
 * 
 * 
 * @author Andy Clement
 * @since 3.0
