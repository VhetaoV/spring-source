 ...
 *
 * <p>
 * ExpressionState用于维护每个表达式评估状态,其他表达式看不到任何更改,但是它给出了一个地方来保存局部变量和复合表达式中的组件表达式进行通信状态这与EvaluationContext相反,这
 * 在表达式评估中是共享的,并且对其的任何更改将被其他表达式或任何选择提出上下文问题的代码所看到。
 * 
 *  <p>它也可以用来定义各种AST节点可能需要的公共实用程序
 * 
 * 
 * @author Andy Clement
 * @since 3.0
...
public class ExpressionState {
	 ...
	 * the scope is exited.
	 * <p>
	 * 一个函数被调用时输入一个新的范围,并且它被用来保存函数调用的参数如果这些参数的名称与更高层次范围内的参数冲突,那么较高层次的范围中的参数将不能被访问正在执行当函数返回时,退出范围
	 */
