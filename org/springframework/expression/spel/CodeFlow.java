 ...
 *
 * <p>
 *  管理由编译过程生成的类它生成字节码时记录中间编译状态它还包括各种字节码生成助手函数
 * 
 * 
 * @author Andy Clement
 * @since 4.1
...
public class CodeFlow implements Opcodes {
	 ...
	 * expression.
	 * <p>
	 * 记录字节码堆栈顶部的类型(即前一个表达式组件的输出类型)新的范围用于评估子表达式,如方法调用表达式中参数值的表达式
	 * 
	 */
	 ...
	 * <p>
	 *  由于调用SpEL ast节点为主要评估方法生成代码,所以可以注册到此类中添加一个字段任何已注册的FieldAdders将在主要评估函数生成完成后被调用
	 * 
	 */
	 ...
	 * <p>
	 *  由于SpEL ast节点被称为为主要评估方法生成代码,因此可以注册添加代码到类中的静态初始化器任何已注册的ClinitAdders将在主要评估函数生成完成后被调用
	 * 
	 */
	 ...
	 * <p>
	 * 正在生成的类的名称通常在生成访问生成的类型上新生成的字段的代码时使用
	 * 
	 */
	 ...
	 * <p>
	 *  当代码生成需要在类级别字段中保存值时,它用于跟踪下一个可用字段ID(用作名称后缀)
	 * 
	 */
	 ...
	 * <p>
	 *  当代码生成需要一个方法中的中间变量时,该方法记录下一个可用变量(变量0是'this')
	 * 
	 */
	 ...
	 * <p>
	 *  推送字节码以加载目标(即作为CompiledExpressiongetValue(target,context)的第一个参数传递的内容)
	 * 
	 * 
	 ...
	 * <p>
	 * 输入新的编译范围,通常是由于嵌套表达式求值。例如,当正在评估方法调用表达式的参数时,每个参数将在一个新的范围内进行求值
	 * 
	 */
	 ...
	 * <p>
	 *  退出一个编译范围,通常在一个嵌套表达式被评估之后例如在一个方法调用的参数被评估之后,这个方法返回到前一个(外部)范围
	 * 
	 */
	 ...
	 * <p>
	 *  如果代码流显示了对javalangBoolean求值的最后一个表达式,那么请插入必要的指令,将其解包到布尔基元
	 * 
	 * 
	 ...
	 * <p>
	 * 在生成主表达式评估方法后调用,该方法将回调任何已注册的FieldAdders或ClinitAdders,以将额外的信息添加到表示编译表达式的类中
	 * 
	 */
	public void finish() {
	 ...
	 * generateCode() method.
	 * <p>
	 *  注册一个FieldAdder,它将向生成的类添加一个新字段,以支持由ast节点生成的代码primary generateCode()方法
	 * 
	 */
	 ...
	 * <p>
	 *  注册一个ClinitAdder,它将生成的类中的静态初始化器添加代码,以支持由ast节点生成的代码primary generateCode()方法
	 * 
	 */
	 ...
	 * trailing semicolon.
	 * <p>
	 *  为方法创建JVM签名描述符由包含括号的方法参数的描述符组成,后面是返回类型的描述符注意这里的描述符是JVM描述符,与编译器使用的其他描述符不同,后缀分号
	 * 
	 * 
	 ...
	 * <p>
	 * 为构造函数创建JVM签名描述符它由包含括号的构造函数参数的描述符组成,后面是返回类型的描述符,它始终为"V"。注意,这里的描述符是JVM描述符,与其他描述符形成不同编译器正在使用哪个不包括尾部分号
	 * 
	 * 
	 ...
	 * Ljava/lang/String)
	 * <p>
	 *  确定指定类的JVM描述符与编译过程中使用的其他描述符不同,这是JVM需要的描述符,因此这包括任何必要的后缀分号(例如Ljava / lang / String;而不是Ljava / lang / St
	 * ring)。
	 * 
	 * 
	 * @param clazz a class
	 ...
	 * <p>
	 * 确定拳击/拆箱是否可以从一种类型到另一种类型假设至少有一种类型是以盒装形式(即单个char描述符)
	 * 
	 * 
	 ...
	 * <p>
	 *  确定所提供的描述符是否支持数字类型或布尔值仅编译过程(当前)支持某些数字类型这些是double,float,long和int
	 * 
	 * 
	 ...
	 * long and int.
	 * <p>
	 *  确定提供的描述符是否为支持的编号仅编译过程(当前)支持某些数字类型这些是double,float,long和int
	 * 
	 * 
	 * @param descriptor the descriptor for a type
	 ...
	 * <p>
	 * 确定特定类型的适当的拳击指示(如果需要拳击),并将指令插入提供的访问者
	 * 
	 * 
	 ...
	 * "I".
	 * <p>
	 *  引用描述符类型描述符就像JVM类型名称,但缺少尾随';'所以对于Object,描述符是"Ljava / lang / Object",对于int是"I"
	 * 
	 * 
	 ...
	 * <p>
	 *  创建一个表示所提供方法的参数类型的描述符数组如果没有参数,则返回一个零大小的数组
	 * 
	 * 
	 ...
	 * <p>
	 *  创建一个表示提供的构造函数的参数类型的描述符数组如果没有参数,返回一个零大小的数组
	 * 
	 * 
	 ...
	 * <p>
	 *  生成适当的字节码以将堆栈项存储在数组中使用的指令取决于类型是原始类型还是参考类型
	 * 
	 * 
	 * @param mv where to insert the bytecode
	 ...
	 * of the array type.
	 * <p>
	 *  生成正确的字节码以构建数组要使用的操作码和与操作码一起传递的签名可能会因阵列类型的签名而异
	 * 
	 * 
	 ...
	 * <p>
	 *  对于在数学运算符中使用的句柄,从堆栈上的(可能是盒装的)数字转换为原始数字类型<p>例如,从整数到双精度,只需要调用'NumberdoubleValue()',而从int到一个双,需要使用字节码'i2
	 * d'。
	 * 
