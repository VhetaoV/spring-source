 ...
 *
 * <p>
 *  使用反射访问属性进行阅读和写入的简单{@link PropertyAccessor}
 * 
 * <p>可以通过public getter方法(读取时)或公共setter方法(写入时)以及公共字段访问属性
 * 
 * 
 * @author Andy Clement
...
public class ReflectivePropertyAccessor implements PropertyAccessor {
	 ...
	 * 'getxY()'.
	 * <p>
	 *  返回给定属性名的方法后缀默认实现使用JavaBean约定,对"xY"形式的属性进行额外支持,其中使用方法'getXY()'优先于"getxY()"的JavaBean约定)
	 * 
	 */
	 ...
	 * <p>
	 *  尝试创建针对特定类别的特定名称的属性定制的优化属性访问器一般ReflectivePropertyAccessor将始终工作,但不是最佳的,因为需要查找每次读取()时使用哪个反射成员(方法/字段)被称为
	 * 此方法将返回ReflectivePropertyAccessor实例,如果它不能构建更优化的东西。
	 * 
	 */
	 ...
	 * <p>
	 *  捕获成员(方法/字段)以反射方式来访问属性值,并且反射调用返回的值的类型描述符
	 * 
	 */
	 ...
	 * <p>
	 * PropertyAccessor的优化形式将使用反射但只知道如何访问特定类的特定属性这与一般的ReflectivePropertyResolver不同,该对象管理可以调用​​以访问不同类的不同属性的方法
	 * /字段缓存。
	 */
