 ...
 *
 * <p>
 * 执行器由解析器构建,并且可以被基础设施缓存,以便快速重复操作,而不会返回到解析器。
 * 例如,在类上运行的特定构造函数可能会被反射构造函数解析器发现 - 然后它将构建一个构造函数执行器执行该构造函数并且ConstructorExecutor可以被重用,而不需要回到解析器再次发现构造函数。
 * 
 *  他们可能会变得陈旧,在这种情况下应该抛出一个AccessException  - 这将导致基础设施返回到解析器要求一个新的
 * 
 * @author Andy Clement
 * @since 3.0
 */
