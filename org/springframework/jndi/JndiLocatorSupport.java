 ...
 *
 * <p>
 *  可以定位任意数量的JNDI对象的类的方便的超类派生自JndiAccessor继承"jndiTemplate"和"jndiEnvironment"bean属性
 * 
 * <p>当访问本地映射(ENC  - 环境命名上下文)资源时,JNDI名称可能包含也可能不包含J2EE应用程序期望的"java：comp / env /"前缀如果没有,"java：comp / env /
 * "如果"resourceRef"属性为true(默认为<strong> false </strong>),并且没有给出其他方案(例如"java："),则前缀将被添加。
 * 
 * 
...
public abstract class JndiLocatorSupport extends JndiAccessor {
	 ...
	 * <p>
	 *  设置是否在J2EE容器中进行查找,即如果JNDI名称尚未包含,则需要添加前缀"java：comp / env /"。
	 * 默认值为"false"<p>注意：只能应用如果没有给出其他方案(例如"java：")。
	 * 
	 */
   ...
   * <p>If the name doesn't begin with "java:comp/env/", this prefix is added
	 ...
	 * <p>
	 * 通过JndiTemplate <p>对给定的名称执行实际的JNDI查找如果名称不以"java：comp / env /"开头,则如果将"resourceRef"设置为"true",则添加该前缀
	 * 
	 * 
	 ...
	 * <p>
	 *  将给定的JNDI名称转换为实际的JNDI名称以使用<p>如果"resourceRef"为"true",则默认实现应用"java：comp / env /"前缀,并且没有给出其他方案(例如"java："
	 * )。
	 * 
