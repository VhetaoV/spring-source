 ...
 *
 * <p>
 *  BeanShell脚本的{@link orgspringframeworkscriptingScriptFactory}实现
 * 
...
public class BshScriptFactory implements ScriptFactory, BeanClassLoaderAware {
	 ...
	 * <p>
	 *  为给定的脚本源创建一个新的BshScriptFactory <p>使用此{@code BshScriptFactory}变体,脚本需要声明一个完整的类或返回脚本对象的实际实例
	 * 
	 * 
	 ...
	 * <p>
	 *  为给定的脚本源创建一个新的BshScriptFactory脚本可能是一个简单的脚本,需要一个相应的代理生成(实现指定的接口),或声明一个完整的类或返回一个实际的脚本对象实例(其中case指定的接口(如
	 * 果有的话)需要由该类/实例实现)。
	 * 
	 * 
	 * @param scriptSourceLocator a locator that points to the source of the script.
