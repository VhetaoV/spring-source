 ...
 *
 * <p>
 * "{@code <lang：groovy />}","{@code <lang：std />}"和"{@code <lang：bsh />}"标签的BeanDefinitionParser实现允许使用动
 * 态语言编写的对象使用{@link orgspringframeworkbeansfactoryBeanFactory}轻松暴露。
 * 
 *  <p>可以将每个对象的脚本指定为对包含该对象的资源的引用(使用"{@code script-source"}属性)或XML配置本身的内联(使用"{@code inline-脚本}'属性
 * 
 *  <p>默认情况下,使用这些标签创建的动态对象为<strong>不</strong>可刷新要启用刷新,请使用"{@code refresh-check-delay"指定每个对象的刷新检查延迟(以毫秒为单位) '属性。
 * 
 * 
 * @author Rob Harrop
 * @author Rod Johnson
...
class ScriptBeanDefinitionParser extends AbstractBeanDefinitionParser {
	 ...
	 * <p>
	 * 该解析器实例将为其创建bean定义的{@link orgspringframeworkscriptingScriptFactory}类
	 * 
	 */
	 ...
	 * <p>
	 *  创建此解析器的新实例,为提供的{@link orgspringframeworkscriptingScriptFactory}类创建bean定义
	 * 
	 * 
	 ...
	 * <p>
	 *  解析动态对象元素并返回生成的bean定义如果需要,请注册{@link ScriptFactoryPostProcessor}
	 * 
	 */
	@Override
	 ...
	 * <p>
	 *  从"{@code script-source}"属性或"{@code inline-script}"元素Logs和{@link XmlReaderContext#error}中解析脚本源,并返回{@code null}
	 * ,如果两者或两者都不这些值被指定。
	 * 
	 */
