...
public class GroovyScriptFactory implements ScriptFactory, BeanFactoryAware, BeanClassLoaderAware {
	 ...
	 * <p>
	 *  为给定的脚本源创建一个新的GroovyScriptFactory <p>我们不需要在这里指定脚本接口,因为Groovy脚本定义了它的Java接口本身
	 * 
	 * 
	 ...
	 * <p>
	 *  为给定的脚本源创建一个新的GroovyScriptFactory,指定可以创建自定义MetaClass以提供缺失方法的策略接口,否则可以更改对象的行为
	 * 
	 * 
	 * @param scriptSourceLocator a locator that points to the source of the script.
	 * Interpreted by the post-processor that actually creates the script.
	 ...
	 * <p>
	 *  为给定的脚本源创建一个新的GroovyScriptFactory,指定一个策略界面,可以在底层的GroovyClassLoader中自定义Groovy的编译过程
	 * 
	 * 
	 * @param scriptSourceLocator a locator that points to the source of the script.
	 * Interpreted by the post-processor that actually creates the script.
	 ...
	 * <p>
	 *  Groovy脚本自己确定它们的接口,因此我们不需要在这里明确地暴露接口
	 * 
	 * 
	 ...
	 * <p>
	 *  Groovy脚本不需要一个配置界面,因为它们作为公共方法公开他们的设置器
	 * 
	 */
	@Override
