 ...
 *
 * <p>
 *  {@link ThemeSource}实现,每个主题查找一个单独的{@link javautilResourceBundle}主题名称被解释为ResourceBundle basename,支持所有主
 * 题的公共基础名称前缀。
 * 
 * 
...
public class ResourceBundleThemeSource implements HierarchicalThemeSource, BeanClassLoaderAware {
	 ...
	 * <p>
	 * 设置应用于ResourceBundle基础名称的前缀,即主题名称Eg：basenamePrefix ="test",themeName ="theme" - > basename ="testtheme
	 * "<p>请注意,ResourceBundle名称实际上是类路径位置：因此,JDK的标准ResourceBundle将点视为包分隔符这意味着"testtheme"实际上等效于"test / theme",
	 * 就像编程的{@code javautilResourceBundle}用法。
	 * 
	 * 
	 ...
	 * <p>
	 *  设置用于解析资源束文件的默认字符集<p> {@ link ResourceBundleMessageSource}的默认值是{@code javautilResourceBundle}默认编码：ISO
	 * -8859-1。
	 * 
	 * 
	 * @since 4.2
	 ...
	 * <p>
	 * 设置是否回退到系统Locale如果没有找到特定Locale的文件<p> {@ link ResourceBundleMessageSource}的默认值为"true"
	 * 
	 * 
	 * @since 4.2
	 ...
	 * <p>
	 *  此实现返回一个SimpleTheme实例,其中包含一个基于ResourceBundle的MessageSource,其基础名称对应于给定的主题名称(以配置的"basenamePrefix"为前缀)。
	 * <p> SimpleTheme实例按主题名称缓存使用可重新加载的MessageSource,如果主题应反映更改底层文件。
	 * 
	 * 
	 * @see #setBasenamePrefix
	 ...
	 * <p>
	 *  为给定的基础名称创建MessageSource,作为相应主题的MessageSource。
	 * <p>默认实现为给定的基础名称创建一个ResourceBundleMessageSource子类可以创建一个特别配置的ReloadableResourceBundleMessageSource,例如。
	 * 
	 * 
