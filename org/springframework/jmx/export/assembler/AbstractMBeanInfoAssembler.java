 ...
 *
 * <p>
 *  {@code MBeanInfoAssembler}接口的抽象实现,封装了{@code ModelMBeanInfo}实例的创建,但将元数据的创建委托给子类
 * 
 * <p>这个类从托管bean实例提供两种类提取：{@link #getTargetClass},提取任何类型的AOP代理之后的目标类,以及{@link #getClassToExpose},返回将会搜索注
 * 释并暴露给JMX运行时。
 * 
 * 
 * @author Rob Harrop
...
public abstract class AbstractMBeanInfoAssembler implements MBeanInfoAssembler {
	 ...
	 * <p>
	 *  创建与所有JMX实现一起提供的{@code ModelMBeanInfoSupport}类的实例,并通过调用子类来填充元数据
	 * 
	 * 
	 ...
	 * <p>
	 *  检查给定的bean实例,如果它不符合此汇编程序的曝光资格,则抛出IllegalArgumentException <p>默认实现为空,接受每个bean实例
	 * 
	 * 
	 * @param managedBean the bean that will be exposed (might be an AOP proxy)
	 ...
	 * <p>
	 * 返回给定bean实例的实际bean类这是暴露于描述样式的JMX属性的类<p>默认实现返回AOP代理的目标类,而纯bean类别
	 * 
	 * 
	 ...
	 * <p>
	 *  返回为给定的bean公开的类或接口这是将搜索属性和操作的类(例如,检查注释)
	 * 
	 * 
	 * @param managedBean the bean instance (might be an AOP proxy)
	 * @return the bean class to expose
	 ...
	 * This is the class that will be searched for attributes and operations
	 * <p>
	 *  返回为给定的bean类公开的类或接口这是将搜索属性和操作的类
	 * 
	 * 
	 ...
	 * <p>
	 *  获取MBean资源的类名<p>默认实现返回一个基于类名称的MBean的简单描述
	 * 
	 * 
	 * @param managedBean the bean instance (might be an AOP proxy)
	 ...
	 * based on the class name.
	 * <p>
	 * 获取MBean资源的描述<p>默认实现返回一个基于类名称的MBean的简单描述
	 * 
	 * 
	 * @param managedBean the bean instance (might be an AOP proxy)
	 * @param beanKey the key associated with the MBean in the beans map
	 ...
	 * <p>
	 *  在{@code ModelMBeanInfo}实例构建之后调用,但在将其传递给{@code MBeanExporter} <p>之前,子类可以实现此方法向MBean元数据添加附加描述符默认实现为空
	 * 
	 * 
	 ...
	 * <p>
	 *  获取MBean资源的构造函数元数据子类应实现此方法以返回应在受管资源的管理接口中公开的所有构造函数的适当元数据。
	 * 缺省实现返回一个空数组{@code ModelMBeanConstructorInfo}。
	 * 
	 * 
	 * @param managedBean the bean instance (might be an AOP proxy)
	 * @param beanKey the key associated with the MBean in the beans map
	 ...
	 * <p>
	 * 获取MBean资源的通知元数据子类应实现此方法以返回适用于受管资源的管理界面中应显示的所有通知的适当元数据<p>默认实现返回一个空数组{@code ModelMBeanNotificationInfo}
	 * 。
	 * 
	 * 
	 * @param managedBean the bean instance (might be an AOP proxy)
	 ...
	 * be exposed in the management interface for the managed resource.
	 * <p>
	 *  获取MBean资源的属性元数据子类应实现此方法以返回适用于所管理资源管理界面中应显示的所有属性的元数据
	 * 
	 * 
	 * @param managedBean the bean instance (might be an AOP proxy)
	 * @param beanKey the key associated with the MBean in the beans map
	 ...
	 * be exposed in the management interface for the managed resource.
	 * <p>
	 *  获取MBean资源的操作元数据子类应实现此方法以返回适用于所管理的管理界面中应显示的所有操作的元数据
	 * 
	 * @param managedBean the bean instance (might be an AOP proxy)
	 * @param beanKey the key associated with the MBean in the beans map
	 * of the {@code MBeanExporter}
