 ...
 *
 * <p>
 *  通用转换器使用约定将源对象转换为{@code targetType},通过委托源对象上的方法或静态工厂方法或{@code targetType}上的构造函数
 * 
 * <h3>转换算法</h3>
 * <ol>
 *  在具有等于{@code targetType}的返回类型的源对象上调用非静态{@code到[targetTypesimpleName]()}方法,如果存在这样的方法例如{@code orgexampleBar Foo# toBar()}
 * 是遵循这个约定的方法<li>否则调用一个<em>静态</em> {@code valueOf(sourceType)}或Java 8 style <em> static </em> {@code of sourceType)}
 * 或{@code from(sourceType)}方法,如果存在这样的方法<li>否则调用{@code targetType}中的一个构造函数,接受一个{@code sourceType}参数,如果这样
 * 
 * @author Keith Donald
