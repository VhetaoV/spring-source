 ...
 *
 * <p>
 *  {@link ObjenesisStd} / {@link ObjenesisBase}的特定于Spring的变体,提供基于{@code Class}键而不是类名的缓存,并允许选择性地使用缓存
 * 
 * 
...
public class SpringObjenesis implements Objenesis {
	 ...
	 * <p>
	 * 指示Spring忽略Objenesis的系统属性,甚至不尝试使用它将此标志设置为"true"等同于让Spring发现Objenesis在运行时不工作,立即触发后退代码路径：最重要的是,这意味着所有CGL
	 * IB AOP代理将通过默认构造函数通过定期实例化创建。
	 * 
	 */
	 ...
	 * <p>
	 * 返回此Objenesis实例是否值得尝试实例创建,即是否尚未使用或已知工作<p>如果配置的Objenesis实例化器策略已被确定为无法在当前JVM上运行,或者如果"springobjenesisigno
	 * re"属性已设置为"true",此方法返回{@code false}。
	 * 
	 */
