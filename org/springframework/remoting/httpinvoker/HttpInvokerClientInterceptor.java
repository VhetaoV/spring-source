 ...
 *
 * <p>
 *  {@link orgaopallianceinterceptMethodInterceptor}用于访问HTTP调用器服务服务URL必须是一个HTTP URL,显示HTTP调用服务器
 * 
 * 序列化远程调用对象并反序列化远程调用结果对象像RMI一样使用Java序列化,但提供了与Caucho基于HTTP的Hessian和Burlap协议相同的设置方式
 * 
 *  HTTP调用器是一个非常可扩展和可定制的协议。它支持RemoteInvocationFactory机制,如RMI调用器,允许包含其他调用属性(例如,安全上下文)。
 * 此外,它允许通过{@link HttpInvokerRequestExecutor }策略。
 * 
 * <p>可以使用JDK的{@link javarmiserverRMIClassLoader}从给定的{@link #setCodebaseUrl代码库}加载类,从远程位置执行按需动态代码下载代码库可以由
 * 多个以空格分隔的URL组成。
 * 请注意RMIClassLoader需要设置一个SecurityManager,类似于使用标准RMI的动态类下载！ (有关详细信息,请参阅RMI文档)。
 * 
 * 
...
public class HttpInvokerClientInterceptor extends RemoteInvocationBasedAccessor
	 ...
	 * <p>
	 * 设置代码库URL从如果在本地找不到可以由多个URL分隔空格<p>遵循RMI的动态类下载的代码库约定与RMI相反,服务器确定类下载的URL(通过" javarmiservercodebase"系统属性),
	 * 客户端在此确定代码库URL服务器通常与服务URL相同,只是指向不同的路径。
	 * 
	 * 
	 * @see #setServiceUrl
	 ...
	 * sophisticated needs.
	 * <p>
	 *  设置用于执行远程调用的HttpInvokerRequestExecutor实现<p>默认是{@link SimpleHttpInvokerRequestExecutor}或者,考虑使用{@link HttpComponentsHttpInvokerRequestExecutor}
	 * 来获得更复杂的需求。
	 * 
	 * 
	 ...
	 * <p>
	 * 返回此远程访问器使用的HttpInvokerRequestExecutor <p>如果没有执行程序已经被初始化,则创建默认的SimpleHttpInvokerRequestExecutor
	 * 
	 */
	 ...
	 * <p>
	 *  通过HttpInvokerRequestExecutor执行给定的远程调用<p>此实现委托给{@link #executeRequest(RemoteInvocation)}可以覆盖以对特定的原始Me
	 * thodInvocation做出反应。
	 * 
	 * 
	 ...
	 * <p>
	 *  通过HttpInvokerRequestExecutor执行给定的远程调用<p>可以在子类中覆盖以将不同的配置对象传递给执行器。
	 * 或者,在此访问器的子类中添加其他配置属性：默认情况下,访问器将自身作为配置对象传递到执行者。
	 * 
	 * 
	 * @param invocation the RemoteInvocation to execute
	 * @return the RemoteInvocationResult object
