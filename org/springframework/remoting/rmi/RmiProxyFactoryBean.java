 ...
 *
 * <p>
 * 支持RMI代理的{@link FactoryBean},支持传统的RMI服务和RMI调用者使用指定的服务接口将代理服务用作bean引用代理将抛出Spring未经检查的远程调用失败的RemoteAcces
 * sException,而不是RMI的RemoteException。
 * 
 *  <p>服务URL必须是有效的RMI URL,如"rmi：// localhost：1099 / myservice"RMI调用者在RmiInvocationHandler级别工作,对于任何服务使用相同
 * 的调用者存根服务接口不必扩展{@code javarmiRemote}或者抛出{@code javarmiRemoteException}当然,进出参数必须是可序列化的。
 * 
 * 使用传统的RMI服务,该代理工厂通常与RMI服务接口一起使用。
 * 
