 ...
 *
 * <p>
 * 特殊的ObjectInputStream子类可以返回到指定的代码库,以从本地找不到加载类与动态类下载的标准RMI约定相反,客户端在此确定代码库URL,而不是"javarmiservercodebase"
 * 系统属性服务器。
 * 
 *  <p>使用JDK的RMIClassLoader从指定的代码库加载类代码库可以由多个URL组成,以空格分隔注意,RMIClassLoader需要设置SecurityManager,就像使用标准RMI的动
 * 态类下载一样！ (有关详细信息,请参阅RMI文档)。
 * 
 * 尽管存在于RMI包中,但是这个类不是用于RmiClientInterceptor,它使用标准的RMI基础架构,因此只能依靠RMI的标准动态类下载"javarmiservercodebase"Codeba
 * seAwareObjectInputStream由HttpInvokerClientInterceptor使用(请参阅"codebaseUrl"属性)。
 * 
 *  感谢Lionel Mestre提出的选择并提供了一个原型！
 * 
...
public class CodebaseAwareObjectInputStream extends ConfigurableObjectInputStream {
	 ...
	 * Create a new CodebaseAwareObjectInputStream for the given InputStream and codebase.
	 * <p>
	 *  为给定的InputStream和codebase创建一个新的CodebaseAwareObjectInputStream
	 * 
	 * 
	 * @param in the InputStream to read from
