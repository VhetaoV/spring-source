 ...
 *
 * <p>
 *  封装远程调用结果,保存结果值或异常用于基于HTTP的序列化调用者
 * 
...
public class RemoteInvocationResult implements Serializable {
	 ...
	 * <p>
	 *  为给定异常创建一个新的RemoteInvocationResult
	 * 
	 * 
	 ...
	 * (e.g. with Jackson).
	 * <p>
	 *  为JavaBean样式反序列化创建一个新的RemoteInvocationResult(例如,使用Jackson)
	 * 
	 * 
	 * @see #setValue
	 * @see #setException
	 ...
	 * <p>
	 *  设置成功调用目标方法返回的结果值(如果有)<p>此setter用于JavaBean样式反序列化使用{@link #RemoteInvocationResult(Object)}否则
	 * 
	 * 
	 ...
	 * <p>
	 *  设置由目标方法调用不成功引发的异常,如果有任何<p>此设置器用于JavaBean样式反序列化使用{@link #RemoteInvocationResult(Throwable)}否则
	 * 
	 * 
	 * @see #RemoteInvocationResult()
	 */
	 ...
	 * <p>
	 *  返回此调用结果是否持有异常如果返回{@code false},则结果值适用(即使是{@code null})
	 * 
	 * 
	 * @see #getValue
	 * @see #getException
	 ...
	 * <p>
	 *  返回此调用结果是否持有InvocationTargetException,由调用目标方法本身引发
	 * 
	 * 
	 * @see #hasException()
	 */
	 ...
	 * <p>
	 * 重新创建调用结果,在成功调用目标方法的情况下返回结果值,或重新抛出目标方法抛出的异常
	 * 
