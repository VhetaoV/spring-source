 ...
 *
 * <p>
 *  实现JDX 6更新4+和Java 7/8中包含的JAXB 21/22的{@code GenericMarshaller}接口
 * 
 * <p>典型的用法是设置这个bean上的"contextPath"或"classesToBeBound"属性,可能通过设置属性,模式,适配器和侦听器来定制编组器和解组器,并引用它
 * 
 * 
...
public class Jaxb2Marshaller implements MimeMarshaller, MimeUnmarshaller, GenericMarshaller, GenericUnmarshaller,
	 ...
	 * <p>
	 *  设置多个JAXB上下文路径由JAXB支持的给定的上下文路径数组转换为冒号分隔的字符串
	 * 
	 */
	 ...
	 * <p>
	 *  设置JAXB上下文路径<p>需要设置此属性{@link #setClassesToBeBound"classesToBeBound"}或{@link #setPackagesToScan"packagesToScan"}
	 * 。
	 * 
	 */
	 ...
	 * or {@link #setPackagesToScan "packagesToScan"} is required.
	 * <p>
	 *  设置要由新创建的JAXBContext识别的Java类的列表<p>需要设置此属性{@link #setContextPath"contextPath"}或{@link #setPackagesToScan"packagesToScan"}
	 * 。
	 * 
	 */
	 ...
	 * <p>
	 *  设置包以在类路径中搜索具有JAXB2注释的类这是使用Spring基础搜索,因此类似于Spring的组件扫描功能({@link orgspringframeworkcontextannotationClassPathBeanDefinitionScanner}
	 ...
	 * <p>
	 *  设置{@code JAXBContext}属性这些实现特定的属性将设置在底层的{@code JAXBContext}
	 * 
	 */
	 ...
	 * <p>
	 * 设置JAXB {@code Marshaller}属性这些属性将在底层的JAXB {@code Marshaller}上设置,并允许诸如缩进等功能
	 * 
	 * 
	 ...
	 * <p>
	 *  设置JAXB {@code Unmarshaller}属性这些属性将在底层JAXB {@code Unmarshaller}上设置
	 * 
	 * 
	 * @param properties the properties
	 ...
	 * <p>
	 *  指定要向JAXB {@code Marshaller}注册的{@code MarshallerListener}
	 * 
	 */
	 ...
	 * <p>
	 *  将{@code UnmarshallerListener}设置为向JAXB {@code Unmarshaller}注册
	 * 
	 */
	 ...
	 * <p>
	 *  设置JAXB验证事件处理程序如果在调用任何元数据API时遇到任何验证错误,JAXB将调用此事件处理程序
	 * 
	 */
	 ...
	 * <p>
	 *  设置模式语言默认是W3C XML Schema：{@code http：// wwww3org / 2001 / XMLSchema"}
	 * 
	 * 
	 ...
	 * <p>
	 *  设置是否为此编组器懒惰地初始化{@link JAXBContext}默认值是{@code false}在启动时初始化;可以切换到{@code true} <p>早期初始化仅适用于{@link #afterPropertiesSet()}
	 * 被调用。
	 * 
	 */
	 ...
	 * <p>
	 *  指定是否启用MTOM支持默认值为{@code false}：使用XOP / MTOM未启用编组
	 * 
	 */
	 ...
	 * <p>
	 * 指定{@link #supports(Class)}为{@link JAXBElement}类返回{@code true} <p> Default为{@code false},这意味着{@code支持(Class)}
	 * 总是返回{ @code false} {@code JAXBElement}类(虽然{@link #supports(Type)}可以返回{@code true},因为它可以获取{@code JAXBElement}
	 * 的类型参数)<p>此属性为通常与{@link orgspringframeworkwebservletviewxmlMarshallingView MarshallingView}之类的使用结合使用,因
	 * 为{@code ModelAndView}在运行时不提供类型参数信息。
	 * 
	 * 
	 * @see #supports(Class)
	 * @see #supports(Type)
	 ...
	 * <p>
	 * 指定{@link #supports(Class)}是否应该检查{@link XmlRootElement @XmlRootElement}注释<p>默认是{@code true},这意味着{@code支持(Class)}
	 * 将检查此注释。
	 *  ,一些JAXB实现(即EclipseLink MOXy)允许在外部定义文件中定义绑定,从而保持类注释免费将此属性设置为{@code false}支持这些JAXB实现。
	 * 
	 * 
	 * @see #supports(Class)
	 * @see #supports(Type)
	 ...
	 * <p>
	 * 指示在解组时是否处理外部XML实体<p>默认值为{@code false},意味着外部实体未解析注意,只有当{@code Source}传递给{@code}时,才会启用/禁用外部实体的处理链接#unma
	 * rshal(Source)}是{@link SAXSource}或{@link StreamSource}对{@link DOMSource}或{@link StAXSource}实例<p> <strong>
	 ...
	 * <p>
	 *  返回此编组器使用的JAXBContext,如果需要,可以懒洋洋建
	 * 
	 */
	 ...
	 * <p>
	 *  检查给定类型是否是JAXB2规范的基本封装类型比较部分851
	 * 
	 */
	 ...
	 * <p>
	 *  返回一个新创建的JAXB编组器JAXB编组器不一定是线程安全的
	 * 
	 */
	 ...
	 * <p>
	 *  模板方法可以被具体的JAXB编组器覆盖,用于自定义初始化行为创建JAXB {@code Marshaller}之后调用,并在相应属性设置后调用<p>默认实现设置{@link #setMarshallerProperties(Map)定义的属性}
	 * ,{@link #setValidationEventHandler(ValidationEventHandler)验证事件处理程序},{@link #setSchemas(Resource [])schemas}
	 * ,{@link #setMarshallerListener(javaxxmlbindMarshallerListener)listener)和{@link #setAdapters XmlAdapter [])适配器}
	 * 。
	 * 
	 */
	 ...
	 * <p>
	 * 返回新创建的JAXB解组器注：JAXB解组器不一定是线程安全的
	 * 
	 */
	 ...
	 * {@link #setAdapters(XmlAdapter[]) adapters}.
	 * <p>
	 *  模板方法可以被具体的JAXB编组器覆盖,用于自定义初始化行为创建JAXB {@code Marshaller}之后调用,并在相应属性设置后调用<p>默认实现设置{@link #setUnmarshallerProperties(Map)定义的属性}
	 * ,{@link #setValidationEventHandler(ValidationEventHandler)验证事件处理程序},{@link #setSchemas(Resource [])schemas}
	 * ,{@link #setUnmarshallerListener(javaxxmlbindUnmarshallerListener)listener)和{@link #setAdapters XmlAdapter [])适配器}
	 * 。
	 * 
	 */
	 ...
	 * <p>
	 *  将给定的{@code JAXBException}转换为{@code orgspringframeworkoxm}层次结构中的适当异常
	 * 
	 * 
