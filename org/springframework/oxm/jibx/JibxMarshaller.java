 ...
 * 
 *  <p>典型的用法是在这个bean上设置{@code targetClass}和可选的{@code bindingName}属性
 * 
 * 
...
public class JibxMarshaller extends AbstractMarshaller implements InitializingBean {
	 ...
	 * <p>
	 * 设置此实例的目标类设置此属性或{@link #setTargetPackage(String)targetPackage}属性是必需的<p>如果设置此属性,则{@link #setTargetPackage(String)targetPackage}
	 * 被忽略。
	 * 
	 */
	 ...
	 * <p>
	 *  设置此实例的目标包需要设置此属性或{@link #setTargetClass(Class)targetClass}属性<p>如果设置了{@link #setTargetClass(Class)targetClass}
	 * ,则此属性将被忽略。
	 * 
	 */
	 ...
	 * <p>
	 * 设置编组时写入的DTD声明的根元素名称默认情况下,这是{@code null}(即,不写DTD声明)<p>如果设置为一个值,则系统ID或公共ID也需要设置
	 * 
	 * 
	 ...
	 * <p>
	 *  设置编组时写入的DTD声明的系统ID默认情况下,这是{@code null}仅在根元素也被设置时使用<p>设置此属性或{@code docTypePublicId},而不是两者
	 * 
	 * 
	 ...
	 * <p>
	 *  设置编组时写入的DTD声明的公共ID默认情况下,这是{@code null}仅在根元素也被设置时使用<p>设置此属性或{@code docTypeSystemId},而不是两者
	 * 
	 * 
	 * @see #setDocTypeRootElementName(String)
	 */
	 ...
	 * By default, this is {@code null}. Only used when the root element also has been set.
	 * <p>
	 * 设置编组时写入的DTD声明的内部子集Id默认情况下,这是{@code null}仅在根元素也被设置时使用
	 * 
	 * 
	 * @see #setDocTypeRootElementName(String)
	 */
	 ...
	 * <p>
	 *  将给定的{@code JiBXException}转换为{@code orgspringframeworkoxm}层次结构中的适当异常<p>布尔标志用于指示在编组或解组期间是否发生此异常,因为JiBX
	 * 本身在其异常中没有区分等级制度。
	 * 
