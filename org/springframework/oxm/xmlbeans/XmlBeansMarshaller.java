 ...
 *
 * <p>
 *  实现Apache XMLBeans的{@link Marshaller}界面
 * 
 * <p>可以通过设置{@code xmlOptions}属性设置选项。提供{@link XmlOptionsFactoryBean}以轻松设置{@link XmlOptions}实例
 * 
 *  <p>可以通过设置{@code validating}属性或直接调用{@link #validate(XmlObject)}方法来验证未编组的对象。
 * 无效对象将导致{@link ValidationFailureException}。
 * 
 *  <p> <b>注意：</b>由于XMLBeans的本质,此编组器要求所有传递的对象都是类型为{@link XmlObject}
 * 
 * 
...
public class XmlBeansMarshaller extends AbstractMarshaller {
	 ...
	 * <p>
	 *  设置这个编组者是否应该验证输入和输出文档默认是{@code false}
	 * 
	 */
	 ...
	 * <p>
	 *  将给定的XMLBeans异常转换为{@code orgspringframeworkoxm}层次结构中的适当异常<p>布尔标志用于指示在编组或解组期间是否发生此异常,因为XMLBeans本身不会在其异
	 * 常层次结构中进行区分。
	 * 
