 ...
 *
 * <p>
 *  Helper类填充{@link javaxmailinternetMimeMessage}
 * 
 * <p>镜像{@link orgspringframeworkmailSimpleMailMessage}的简单设置器,直接将值应用于底层的MimeMessage允许为整个消息定义字符编码,并由该帮助类的
 * 所有方法自动应用。
 * 
 *  <p>支持HTML文本内容,内联元素(如图像和典型邮件附件)还支持邮件地址附带的个人名称请注意,高级设置仍可直接应用于底层的MimeMessage对象！
 * 
 * 通常在{@link MimeMessagePreparator}实现或{@link JavaMailSender}客户端代码中使用：简单地将其实例化为MimeMessage包装器,在包装器上调用sett
 * er,使用底层MimeMessage进行邮件发送也由{@link JavaMailSenderImpl }。
 * 
 *  <p>带有内联图片和PDF附件的HTML邮件的示例代码：
 * 
 * <pre class="code">
 * mailSendersend(新的MimeMessagePreparator(){public void prepare(MimeMessage mimeMessage)throws MessagingException {MimeMessageHelper message = new MimeMessageHelper(mimeMessage,true,"UTF-8"); messagesetFrom("me @ mailcom"); messagesetTo("you @ mailcom" ); messageetSubject("my subject"); messagesetText("my text&lt; img src ='cid：myLogo'&gt;",true); messageaddInline("myLogo",new ClassPathResource("img / mylogogif")); messageaddAttachment ("myDocumentpdf",新的ClassPathResource("doc / myDocumentpdf"));}
 * }); </pre>。
 * 
 * 考虑使用{@link MimeMailMessage}(它实现了普通的{@link orgspringframeworkmailMailMessage}接口,就像{@link orgspringframeworkmailSimpleMailMessage}
 * 一样),以帮助消息人口代码与简单的消息或MIME消息交互一个通用接口。
 * 
 * <p> <b>关于多部分邮件的警告：</b>只包含HTML文本但不包含内联元素或附件的简单MIME邮件可以在或多或少的使用能够进行HTML呈现的电子邮件客户端。
 * 但是,内联元素和附件仍然是电子邮件客户端之间的主要兼容性问题：实际上无法获取在Microsoft Outlook,Lotus Notes和Mac Mail中工作的内联元素和附件。
 * 请考虑为您的需要选择特定的多部分模式：MULTIPART_MODE常量上的javadoc包含更详细的信息。
 * 
 * 
...
public class MimeMessageHelper {
	 ...
	 * <p>
	 * 指示具有"混合"类型的单个根多部分元素的多部分消息的常量文本,内联元素和附件将全部添加到该根元素<p>这是Spring 10的默认行为已知在Outlook上正常工作但是,其他邮件客户端倾向于将内联元素误
	 * 解为附件和/或内联显示附件。
	 * 
	 */
	 ...
	 * <p>
	 * 指示具有"相关"类型的单个根multipart元素的多部分消息的常量文本,内联元素和附件将被添加到该根元素<p>这是从Spring 11到12最后的默认行为这是"Microsoft多部门模式",由Out
	 * look本机发送已知可以在Outlook,Outlook Express,Yahoo Mail上正常工作,并且在很大程度上也可以在Mac Mail上(附带列出的内联元素附加附件,尽管内置元素也显示inl
	 * ine)在Lotus Notes上无法正常工作(附件不会在那里显示)。
	 * 
	 */
	 ...
	 * <p>
	 * 指示具有根多部分元素"mixed"的多部分消息的常量加上"相关"类型的嵌套多部分元素文本和内联元素将被添加到嵌套的"相关"元素中,而附件将被添加到"混合"根元素<p>这是Spring 121之后的默认值
	 * 这根据MIME规则可以说是最正确的MIME结构：已知在Outlook,Outlook Express,Yahoo Mail和Lotus Notes上正常工作在Mac Mail上无法正常工作如果您在Mac
	 * 上指定Mac Mail或遇到特定邮件的问题,请考虑使用MULTIPART_MODE_RELATED。
	 * 
	 */
	 ...
	 * <p>
	 * 为给定的MimeMessage创建一个新的MimeMessageHelper,假设一个简单的文本消息(没有多部分内容,即没有替代文本,没有内联元素或附件)消息的字符编码将从传入的MimeMessage对
	 * 象中获取,如果携带,否则,将使用JavaMail的默认编码。
	 * 
	 * 
	 ...
	 * i.e. no alternative texts and no inline elements or attachments).
	 * <p>
	 *  为给定的MimeMessage创建一个新的MimeMessageHelper,假设一个简单的文本消息(没有多部分内容,即没有替代文本,没有内联元素或附件)
	 * 
	 * 
	 * @param mimeMessage MimeMessage to work on
	 ...
	 * JavaMail's default encoding will be used.
	 * <p>
	 * 为多个模式(支持替代文本,内联元素和附件)创建一个新的MimeMessageHelper,如果请求,请考虑使用MimeMessageHelper构造函数,该构造函数需要multipartMode参数来选
	 * 择除MULTIPART_MODE_MIXED_RELATED之外的特定多部分模式<p>消息的字符编码将从传入的MimeMessage对象中获取,如果携带在其中,则将使用JavaMail的默认编码。
	 * 
	 * 
	 * @param mimeMessage MimeMessage to work on
	 ...
	 * mode other than MULTIPART_MODE_MIXED_RELATED.
	 * <p>
	 *  创建一个新的MimeMessageHelper给定的MimeMessage,在多部分模式(支持替代文本,内联元素和附件)(如果请求)<p>考虑使用MimeMessageHelper构造函数,该构造函数
	 * 需要multipartMode参数来选择除MULTIPART_MODE_MIXED_RELATED之外的特定多部分模式。
	 * 
	 * 
	 * @param mimeMessage MimeMessage to work on
	 * @param multipart whether to create a multipart message that
	 ...
	 * JavaMail's default encoding will be used.
	 * <p>
	 * 为给定的MimeMessage创建一个新的MimeMessageHelper,在多部分模式下(支持替代文本,内联元素和附件)(如果请求)消息的字符编码将从传入的MimeMessage对象中获取,如果在其
	 * 中存在,则为JavaMail将使用默认编码。
	 * 
	 * 
	 * @param mimeMessage MimeMessage to work on
	 ...
	 * elements and attachments) if requested.
	 * <p>
	 *  为给定的MimeMessage创建一个新的MimeMessageHelper,在多部分模式下(支持替代文本,内联元素和附件)
	 * 
	 * 
	 * @param mimeMessage MimeMessage to work on
	 * @param multipartMode which kind of multipart message to create
	 ...
	 * <p>
	 * 确定要使用的MimeMultipart对象,一方面用于存储附件,另一方面用于文本和内联元素<p>文本和内联元素可以存储在根元素本身中(MULTIPART_MODE_MIXED,MULTIPART_MOD
	 * E_RELATED )或直接在嵌套元素而不是根元素中(MULTIPART_MODE_MIXED_RELATED)<p>默认情况下,根MimeMultipart元素将为"mixed"(MULTIPART_
	 * MODE_MIXED)或"related"(MULTIPART_MODE_RELATED)类型。
	 * 将会添加主multipart元素作为类型"相关"(MULTIPART_MODE_MIXED_RELATED)的嵌套元素或与根元素本身相同(MULTIPART_MODE_MIXED,MULTIPART_M
	 * ODE_RELATED)。
	 * 
	 * 
	 ...
	 * <p>
	 *  设置给此MimeMultipart对象供该MimeMessageHelper使用
	 * 
	 * 
	 ...
	 * <p>
	 *  如果此帮助器未处于多部分模式,则抛出IllegalStateException异常
	 * 
	 */
	 ...
	 * <p>
	 *  返回根MIME"multipart / mixed"对象(如果有的话)可以用于手动添加附件<p>这将是MimeMessage的直接内容,如果是多部分邮件
	 * 
	 * 
	 ...
	 * in case of a multipart mail.
	 * <p>
	 *  返回底层的MIME"multipart / related"对象(如果有的话)可以用于手动添加正文部分,内联元素等<p>这将嵌套在根MimeMultipart的根目录中,以便在多部分邮件的情况下
	 * 
	 * 
	 * @throws IllegalStateException if this helper is not in multipart mode
	 * @see #isMultipart
	 ...
	 * <p>
	 * 设置Java激活框架{@code FileTypeMap}用于确定添加到消息中的内联内容和附件的内容类型默认是底层MimeMessage携带的{@code FileTypeMap},如果有的话,或Act
	 * ivation Framework的默认{@code FileTypeMap}实例。
	 * 
	 * 
	 * @see #addInline
	 * @see #addAttachment
	 ...
	 * <p>
	 *  设置是否验证传递给该帮助者的所有地址默认值为"false"<p>请注意,默认情况下,这是仅适用于JavaMail> = 13您可以覆盖默认的{@code validateAddress方法}以验证旧J
	 * avaMail版本(或用于自定义验证)。
	 * 
	 * 
	 * @see #validateAddress
	 ...
	 * <p>
	 * 验证给定的邮件地址由MimeMessageHelper的所有地址设置器和加法器调用<p>默认实现调用{@code InternetAddressvalidate()},前提是帮助程序实例激活了地址验证<p>
	 * 请注意,此方法将仅适用于JavaMail > = 13您可以将其替换为对旧JavaMail版本进行验证或进行自定义验证。
	 * 
	 * 
	 ...
	 * <p>
	 *  验证所有给定的邮件地址默认实现简单地委托给每个地址的validateAddress
	 * 
	 * 
	 ...
	 * <p>
	 * 将指定的文本直接设置为非多部分模式的内容或多部分模式中的默认主体部分始终应用默认内容类型"text / plain"<p> <b>注意：</b>调用{@link #addInline} <i> afte
	 * r </i> {@code setText};否则,邮件阅读器可能无法正确解析内联引用。
	 * 
	 * 
	 ...
	 * else, mail readers might not be able to resolve inline references correctly.
	 * <p>
	 *  将指定的文本直接设置为非多部分模式中的内容或多部分模式中的默认主体部分"html"标志确定要应用的内容类型<p> <b>注意：</b>调用{@link #addInline} <i> after </i>
	 *  {@code setText};否则,邮件阅读器可能无法正确解析内联引用。
	 * 
	 * 
	 * @param text the text for the message
	 ...
	 * else, mail readers might not be able to resolve inline references correctly.
	 * <p>
	 * 将给定的纯文本和HTML文本设置为替代方案,为电子邮件客户端提供这两个选项需要多部分模式<p> <b>注意：</b>在</i>之后调用{@link #addInline} <i>代码setText};否
	 * 则,邮件阅读器可能无法正确解析内联引用。
	 * 
	 * 
	 ...
	 * else, mail readers might not be able to resolve inline references correctly.
	 * <p>
	 *  将内联元素添加到MimeMessage中,从{@code javaxactivationDataSource}中获取内容<p>请注意,DataSource实现返回的InputStream需要在每次调用
	 * 时都是<i>新的一个</i>,因为JavaMail将多次调用{@code getInputStream()} <p> <b>注意：</b>在</i> {@link #setText}之后调用{@code addInline}
	 *  <i>否则,邮件阅读器可能无法正确解析内联引用。
	 * 
	 * 
	 ...
	 * else, mail readers might not be able to resolve inline references correctly.
	 * <p>
	 * 将内联元素添加到MimeMessage中,从{@code javaioFile} <p>中获取内容内容类型将由给定内容文件的名称确定不要将其用于具有任意文件名的临时文件(可能以" tmp"等)！ <p>
	 *  <b>注意：</b>在</i> {@link #setText}之后调用{@code addInline} <i>否则,邮件阅读器可能无法正确解析内联引用。
	 * 
	 * 
	 * @param contentId the content ID to use. Will end up as "Content-ID" header
	 * in the body part, surrounded by angle brackets: e.g. "myId" -> "&lt;myId&gt;".
	 ...
	 * else, mail readers might not be able to resolve inline references correctly.
	 * <p>
	 * 将内联元素添加到MimeMessage中,从{@code orgspringframeworkcoreioResource}中获取内容<p>内容类型将由给定内容文件的名称确定不要将其用于具有任意文件名的
	 * 临时文件(可能以" tmp"等)！ <p>请注意,由于JavaMail将多次调用{@code getInputStream()},所以Resource实现所返回的InputStream必须是每个调用的<i>
	 * 新的</i>。
	 * 注意： </b>在</i> {@link #setText}之后调用{@code addInline} <i>否则,邮件阅读器可能无法正确解析内联引用。
	 * 
	 * 
	 * @param contentId the content ID to use. Will end up as "Content-ID" header
	 * in the body part, surrounded by angle brackets: e.g. "myId" -> "&lt;myId&gt;".
	 ...
	 * else, mail readers might not be able to resolve inline references correctly.
	 * <p>
	 * 将内联元素添加到MimeMessage,从{@code orgspringframeworkcoreInputStreamResource}中获取内容,并明确指定内容类型<p>您可以通过Java Act
	 * ivation Framework的FileTypeMap来确定任何给定文件名的内容类型,例如持有的请注意,InputStreamSource实现返回的InputStream需要在每次调用时都是<i>新
	 * 的一个,因为JavaMail会多次调用{@code getInputStream()} <b> <b >注意：</b>在</i> {@code setText}之后调用{@code addInline}
	 *  <i>否则,邮件阅读器可能无法正确解析内联引用。
	 * 
	 * 
	 * @param contentId the content ID to use. Will end up as "Content-ID" header
	 ...
	 * {@code getInputStream()} multiple times.
	 * <p>
	 * 添加一个附件到MimeMessage,从{@code javaxactivationDataSource} <p>中获取内容。
	 * 请注意,DataSource实现返回的InputStream需要是每个调用的<i>新的一个</i>,因为JavaMail将调用{@code getInputStream()}多次。
	 * 
	 * 
	 ...
	 * filenames (possibly ending in ".tmp" or the like)!
	 * <p>
	 *  添加一个附件到MimeMessage,从{@code javaioFile} <p>中获取内容内容类型将由给定内容文件的名称确定不要将此用于具有任意文件名的临时文件(可能以"tmp"结尾) "等等)！
	 * 。
	 * 
	 * 
	 * @param attachmentFilename the name of the attachment as it will
	 ...
	 * <p>
	 * 将附件添加到MimeMessage中,从{@code orgspringframeworkcoreioInputStreamResource}中获取内容<p>内容类型将由附件的给定文件名确定。
	 * 因此,任何内容源都将是正常的,包括具有任意文件名的临时文件<p >请注意,InputStreamSource实现返回的InputStream需要在每次调用时都是<i>新的一个,因为JavaMail会多次
	 * 调用{@code getInputStream()}。
	 * 将附件添加到MimeMessage中,从{@code orgspringframeworkcoreioInputStreamResource}中获取内容<p>内容类型将由附件的给定文件名确定。
	 * 
	 * 
	 * @param attachmentFilename the name of the attachment as it will
	 ...
	 * JavaMail will invoke {@code getInputStream()} multiple times.
	 * <p>
	 *  添加一个附件到MimeMessage,从{@code orgspringframeworkcoreioInputStreamResource} <p>中获取内容。
	 * 请注意,InputStreamSource实现返回的InputStream需要是每个调用的新的</i>,因为JavaMail将调用{@code getInputStream()}多次。
	 * 
	 * 
	 * @param attachmentFilename the name of the attachment as it will
	 * appear in the mail
