...
public class MailSendException extends MailException {
	 ...
	 * <p>
	 * 用于注册失败消息的构造方法,其中作为键失败的消息和抛出的异常作为值<p>消息应该与最初传递给调用的send方法相同
	 * 
	 * 
	 * @param msg the detail message
	 * @param cause the root cause from the mail API in use
	 ...
	 * <p>
	 * 将失败消息的映射返回为键,并将抛出的异常作为值<p>注意,通用邮件服务器连接失败不会导致此处返回失败的消息：仅当实际发送尝试发送消息时才会包含该消息但失败<p>消息将与原始传递给调用的send方法相同,
	 * 即在使用通用MailSender接口的情况下为SimpleMailMessages <p>如果通过JavaMailSender发送MimeMessage实例,消息将为键入MimeMessage <p> 
	 * <b>注意：</b>此序列化后无法使用此映射在这种情况下使用{@link #getMessageExceptions()},这在序列化后也可用。
	 * 
	 * 
