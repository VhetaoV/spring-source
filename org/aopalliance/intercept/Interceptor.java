 ...
 *  此接口表示通用拦截器
 * 
 * 通用拦截器可以拦截基础程序中发生的运行时事件这些事件通过(重新连接)连接点来实现运行时连接点可以是调用,现场访问,异常
 * 
 *  <p>此接口不直接使用使用子接口拦截特定事件例如,以下类实现一些特定的拦截器以实现调试器：
 * 
 * <pre class=code>
 *  类DebuggingInterceptor实现了MethodInterceptor,ConstructorInterceptor,FieldInterceptor {
 * 
 *  对象调用(MethodInvocation i)throws Throwable {debug(igetMethod(),igetThis(),igetArgs()); return iproceed(); }
 * 。
 * 
 * 
 * @author Rod Johnson
